...
void mergeable::defaultAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
...
namespace adaptive{
float* refinementRatio::ptr = nullptr;
size_t refinementRatio::alloc_size = 0;


void refinementRatio::defaultAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(float);
	cudaAllocateMemory(&ptr, alloc_size);
}
void refinementRatio::leanAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(float);
	
}

void refinementRatio::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
}
void refinementRatio::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
}
refinementRatio::operator type*(){ return ptr;}
float& refinementRatio::operator[](size_t idx){ return ptr[idx];}
bool refinementRatio::valid(){
	bool condition = true;
	condition = condition && get<parameters::modules::adaptive>() == true;
	return condition;
}
...
namespace adaptive{
int32_t* mergeIndicator::ptr = nullptr;
size_t mergeIndicator::alloc_size = 0;

int32_t* mergeIndicator::rear_ptr = nullptr;
void mergeIndicator::swap() { std::swap(ptr, rear_ptr); }

void mergeIndicator::defaultAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(int32_t);
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void mergeIndicator::leanAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(int32_t);
...
void mergeIndicator::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void mergeIndicator::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
	cudaFree(rear_ptr);
...
mergeIndicator::operator type*(){ return ptr;}
int32_t& mergeIndicator::operator[](size_t idx){ return ptr[idx];}
bool mergeIndicator::valid(){
	bool condition = true;
	condition = condition && get<parameters::modules::adaptive>() == true;
	return condition;
}
...
namespace advectionArrays{
float4* vorticity::ptr = nullptr;
size_t vorticity::alloc_size = 0;

float4* vorticity::rear_ptr = nullptr;
void vorticity::swap() { std::swap(ptr, rear_ptr); }

void vorticity::defaultAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(float4);
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void vorticity::leanAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(float4);
...
void vorticity::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
...
vorticity::operator type*(){ return ptr;}
float4& vorticity::operator[](size_t idx){ return ptr[idx];}
bool vorticity::valid(){
	bool condition = false;
...
namespace basicArrays{
int* optimization_lifetime::ptr = nullptr;
size_t optimization_lifetime::alloc_size = 0;

int* optimization_lifetime::rear_ptr = nullptr;
void optimization_lifetime::swap() { std::swap(ptr, rear_ptr); }

void optimization_lifetime::defaultAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(int);
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void optimization_lifetime::leanAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(int);
...
void optimization_lifetime::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void optimization_lifetime::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
	cudaFree(rear_ptr);
...
optimization_lifetime::operator type*(){ return ptr;}
int& optimization_lifetime::operator[](size_t idx){ return ptr[idx];}
bool optimization_lifetime::valid(){
	bool condition = true;
	return condition;
}
}
namespace basicArrays{
int* optimization_lifetime_merge::ptr = nullptr;
size_t optimization_lifetime_merge::alloc_size = 0;

int* optimization_lifetime_merge::rear_ptr = nullptr;
void optimization_lifetime_merge::swap() { std::swap(ptr, rear_ptr); }

void optimization_lifetime_merge::defaultAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(int);
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void optimization_lifetime_merge::leanAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(int);
...
void optimization_lifetime_merge::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void optimization_lifetime_merge::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
	cudaFree(rear_ptr);
...
optimization_lifetime_merge::operator type*(){ return ptr;}
int& optimization_lifetime_merge::operator[](size_t idx){ return ptr[idx];}
bool optimization_lifetime_merge::valid(){
	bool condition = true;
	return condition;
}
}
...
namespace basicArrays{
int* optimization_group::ptr = nullptr;
size_t optimization_group::alloc_size = 0;

int* optimization_group::rear_ptr = nullptr;
void optimization_group::swap() { std::swap(ptr, rear_ptr); }

void optimization_group::defaultAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(int);
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void optimization_group::leanAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(int);
...
void optimization_group::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void optimization_group::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
	cudaFree(rear_ptr);
...
optimization_group::operator type*(){ return ptr;}
int& optimization_group::operator[](size_t idx){ return ptr[idx];}
bool optimization_group::valid(){
	bool condition = true;
	return condition;
}
}
...
namespace basicArrays{
float* old_density::ptr = nullptr;
size_t old_density::alloc_size = 0;

float* old_density::rear_ptr = nullptr;
void old_density::swap() { std::swap(ptr, rear_ptr); }

void old_density::defaultAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(float);
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void old_density::leanAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(float);
...
void old_density::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void old_density::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
	cudaFree(rear_ptr);
...
old_density::operator type*(){ return ptr;}
float& old_density::operator[](size_t idx){ return ptr[idx];}
bool old_density::valid(){
	bool condition = true;
	return condition;
}
}
namespace basicArrays{
float* density_b::ptr = nullptr;
size_t density_b::alloc_size = 0;

float* density_b::rear_ptr = nullptr;
void density_b::swap() { std::swap(ptr, rear_ptr); }

void density_b::defaultAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(float);
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void density_b::leanAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(float);
...
void density_b::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
...
density_b::operator type*(){ return ptr;}
float& density_b::operator[](size_t idx){ return ptr[idx];}
bool density_b::valid(){
	bool condition = true;
	return condition;
}
}
namespace basicArrays{
float* density_f::ptr = nullptr;
size_t density_f::alloc_size = 0;

float* density_f::rear_ptr = nullptr;
void density_f::swap() { std::swap(ptr, rear_ptr); }

void density_f::defaultAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(float);
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void density_f::leanAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(float);
...
void density_f::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
...
density_f::operator type*(){ return ptr;}
float& density_f::operator[](size_t idx){ return ptr[idx];}
bool density_f::valid(){
	bool condition = true;
	return condition;
}
}
...
void matrixTest::defaultAllocate(){
	auto elems = (1);
	alloc_size = (math::max(1,(int32_t) get<parameters::rigid_volumes::volume>().size())) * elems * sizeof(Matrix4x4);
	cudaAllocateMemory(&ptr, alloc_size);
}
...
namespace basicArrays{
float* sdf_gridsize::ptr = nullptr;
size_t sdf_gridsize::alloc_size = 0;


void sdf_gridsize::defaultAllocate(){
	auto elems = (1);
	alloc_size = (math::max(1,(int32_t) get<parameters::rigid_volumes::volume>().size())) * elems * sizeof(float);
	cudaAllocateMemory(&ptr, alloc_size);
}
void sdf_gridsize::leanAllocate(){
	auto elems = (1);
	alloc_size = (math::max(1,(int32_t) get<parameters::rigid_volumes::volume>().size())) * elems * sizeof(float);
...
void sdf_gridsize::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
}
void sdf_gridsize::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
}
sdf_gridsize::operator type*(){ return ptr;}
float& sdf_gridsize::operator[](size_t idx){ return ptr[idx];}
bool sdf_gridsize::valid(){
	bool condition = true;
	return condition;
}
}
namespace basicArrays{
float4* sdf_minpoint2::ptr = nullptr;
size_t sdf_minpoint2::alloc_size = 0;


void sdf_minpoint2::defaultAllocate(){
	auto elems = (1);
	alloc_size = (math::max(1,(int32_t) get<parameters::rigid_volumes::volume>().size())) * elems * sizeof(float4);
	cudaAllocateMemory(&ptr, alloc_size);
}
void sdf_minpoint2::leanAllocate(){
	auto elems = (1);
	alloc_size = (math::max(1,(int32_t) get<parameters::rigid_volumes::volume>().size())) * elems * sizeof(float4);
...
void sdf_minpoint2::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
}
void sdf_minpoint2::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
}
sdf_minpoint2::operator type*(){ return ptr;}
float4& sdf_minpoint2::operator[](size_t idx){ return ptr[idx];}
bool sdf_minpoint2::valid(){
	bool condition = true;
	return condition;
}
}
namespace basicArrays{
float3* sdf_minpoint1::ptr = nullptr;
size_t sdf_minpoint1::alloc_size = 0;


void sdf_minpoint1::defaultAllocate(){
	auto elems = (1);
	alloc_size = (math::max(1,(int32_t) get<parameters::rigid_volumes::volume>().size())) * elems * sizeof(float3);
	cudaAllocateMemory(&ptr, alloc_size);
}
void sdf_minpoint1::leanAllocate(){
	auto elems = (1);
	alloc_size = (math::max(1,(int32_t) get<parameters::rigid_volumes::volume>().size())) * elems * sizeof(float3);
...
void sdf_minpoint1::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
}
void sdf_minpoint1::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
}
sdf_minpoint1::operator type*(){ return ptr;}
float3& sdf_minpoint1::operator[](size_t idx){ return ptr[idx];}
bool sdf_minpoint1::valid(){
	bool condition = true;
	return condition;
}
}
...
namespace basicArrays{
float4* acceleration_inter::ptr = nullptr;
size_t acceleration_inter::alloc_size = 0;

float4* acceleration_inter::rear_ptr = nullptr;
void acceleration_inter::swap() { std::swap(ptr, rear_ptr); }

void acceleration_inter::defaultAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(float4);
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void acceleration_inter::leanAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(float4);
...
void acceleration_inter::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void acceleration_inter::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
	cudaFree(rear_ptr);
...
acceleration_inter::operator type*(){ return ptr;}
float4& acceleration_inter::operator[](size_t idx){ return ptr[idx];}
bool acceleration_inter::valid(){
	bool condition = true;
	return condition;
}
}
...
namespace basicArrays{
float* rigidbody_sdf::ptr = nullptr;
size_t rigidbody_sdf::alloc_size = 0;



void rigidbody_sdf::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
}
void rigidbody_sdf::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
}
rigidbody_sdf::operator type*(){ return ptr;}
float& rigidbody_sdf::operator[](size_t idx){ return ptr[idx];}
bool rigidbody_sdf::valid(){
	bool condition = true;
	return condition;
}
}
namespace basicArrays{
float* rigidbody_sdf2::ptr = nullptr;
size_t rigidbody_sdf2::alloc_size = 0;

float* rigidbody_sdf2::rear_ptr = nullptr;
void rigidbody_sdf2::swap() { std::swap(ptr, rear_ptr); }

void rigidbody_sdf2::defaultAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(float);
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void rigidbody_sdf2::leanAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(float);
...
void rigidbody_sdf2::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void rigidbody_sdf2::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
	cudaFree(rear_ptr);
...
rigidbody_sdf2::operator type*(){ return ptr;}
float& rigidbody_sdf2::operator[](size_t idx){ return ptr[idx];}
bool rigidbody_sdf2::valid(){
	bool condition = true;
	return condition;
}
}
...
namespace basicArrays{
int* tobeoptimized::ptr = nullptr;
size_t tobeoptimized::alloc_size = 0;

int* tobeoptimized::rear_ptr = nullptr;
void tobeoptimized::swap() { std::swap(ptr, rear_ptr); }

void tobeoptimized::defaultAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(int);
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void tobeoptimized::leanAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(int);
...
void tobeoptimized::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void tobeoptimized::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
	cudaFree(rear_ptr);
...
tobeoptimized::operator type*(){ return ptr;}
int& tobeoptimized::operator[](size_t idx){ return ptr[idx];}
bool tobeoptimized::valid(){
	bool condition = true;
	return condition;
}
}
namespace basicArrays{
int* uid::ptr = nullptr;
size_t uid::alloc_size = 0;

int* uid::rear_ptr = nullptr;
void uid::swap() { std::swap(ptr, rear_ptr); }

void uid::defaultAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(int);
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void uid::leanAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(int);
...
uid::operator type*(){ return ptr;}
int& uid::operator[](size_t idx){ return ptr[idx];}
bool uid::valid(){
	bool condition = true;
	return condition;
}
...
void volumeBoundaryTransformMatrixInverse::defaultAllocate(){
	auto elems = (1);
	alloc_size = math::max(1u,(uint32_t)get<parameters::boundary_volumes::volume>().size()) * elems * sizeof(Matrix4x4);
	cudaAllocateMemory(&ptr, alloc_size);
}
...
void volumeBoundaryAcceleration::defaultAllocate(){
	auto elems = (1);
	alloc_size = math::max(1u,(uint32_t)get<parameters::boundary_volumes::volume>().size()) * elems * sizeof(float4);
	cudaAllocateMemory(&ptr, alloc_size);
}
...
void volumeBoundaryVolumes::defaultAllocate(){
	auto elems = (1);
	alloc_size = math::max(1u,(uint32_t)get<parameters::boundary_volumes::volume>().size()) * elems * sizeof(cudaTextureObject_t);
	cudaAllocateMemory(&ptr, alloc_size);
}
...
namespace boundaryArrays{
gvdb::VDBInfo** volumeBoundaryGVDBVolumes::ptr = nullptr;
size_t volumeBoundaryGVDBVolumes::alloc_size = 0;


void volumeBoundaryGVDBVolumes::defaultAllocate(){
	auto elems = (1);
	alloc_size = math::max(1u,(uint32_t)get<parameters::boundary_volumes::volume>().size()) * elems * sizeof(gvdb::VDBInfo*);
	cudaAllocateMemory(&ptr, alloc_size);
}
void volumeBoundaryGVDBVolumes::leanAllocate(){
	auto elems = (1);
	alloc_size = math::max(1u,(uint32_t)get<parameters::boundary_volumes::volume>().size()) * elems * sizeof(gvdb::VDBInfo*);
...
void volumeBoundaryGVDBVolumes::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
}
void volumeBoundaryGVDBVolumes::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
}
volumeBoundaryGVDBVolumes::operator type*(){ return ptr;}
gvdb::VDBInfo*& volumeBoundaryGVDBVolumes::operator[](size_t idx){ return ptr[idx];}
bool volumeBoundaryGVDBVolumes::valid(){
	bool condition = true;
	return condition;
}
}
namespace boundaryArrays{
float4* gvdbOffsets::ptr = nullptr;
size_t gvdbOffsets::alloc_size = 0;


void gvdbOffsets::defaultAllocate(){
	auto elems = (1);
	alloc_size = math::max(1u,(uint32_t)get<parameters::boundary_volumes::volume>().size()) * elems * sizeof(float4);
	cudaAllocateMemory(&ptr, alloc_size);
}
void gvdbOffsets::leanAllocate(){
	auto elems = (1);
	alloc_size = math::max(1u,(uint32_t)get<parameters::boundary_volumes::volume>().size()) * elems * sizeof(float4);
...
void gvdbOffsets::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
}
void gvdbOffsets::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
}
gvdbOffsets::operator type*(){ return ptr;}
float4& gvdbOffsets::operator[](size_t idx){ return ptr[idx];}
bool gvdbOffsets::valid(){
	bool condition = true;
	return condition;
}
}
namespace boundaryArrays{
float4* gvdbVoxelSizes::ptr = nullptr;
size_t gvdbVoxelSizes::alloc_size = 0;


void gvdbVoxelSizes::defaultAllocate(){
	auto elems = (1);
	alloc_size = math::max(1u,(uint32_t)get<parameters::boundary_volumes::volume>().size()) * elems * sizeof(float4);
	cudaAllocateMemory(&ptr, alloc_size);
}
void gvdbVoxelSizes::leanAllocate(){
	auto elems = (1);
	alloc_size = math::max(1u,(uint32_t)get<parameters::boundary_volumes::volume>().size()) * elems * sizeof(float4);
...
void gvdbVoxelSizes::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
}
void gvdbVoxelSizes::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
}
gvdbVoxelSizes::operator type*(){ return ptr;}
float4& gvdbVoxelSizes::operator[](size_t idx){ return ptr[idx];}
bool gvdbVoxelSizes::valid(){
	bool condition = true;
	return condition;
}
}
...
void volumeBoundaryDimensions::defaultAllocate(){
	auto elems = (1);
	alloc_size = math::max(1u,(uint32_t)get<parameters::boundary_volumes::volume>().size()) * elems * sizeof(int4);
	cudaAllocateMemory(&ptr, alloc_size);
}
...
void volumeBoundaryDensity::defaultAllocate(){
	auto elems = (1);
	alloc_size = math::max(1u,(uint32_t)get<parameters::boundary_volumes::volume>().size()) * elems * sizeof(float);
	cudaAllocateMemory(&ptr, alloc_size);
}
...
void volumeBoundaryKind::defaultAllocate(){
	auto elems = (1);
	alloc_size = math::max(1u,(uint32_t)get<parameters::boundary_volumes::volume>().size()) * elems * sizeof(int32_t);
	cudaAllocateMemory(&ptr, alloc_size);
}
...
bool _sourceTerm::valid(){
	...
	condition = condition || get<parameters::modules::pressure>() == "IISPH17_PRESSURE_MIRRORING_D";
	condition = condition || get<parameters::modules::pressure>() == "IISPH17_PRESSURE_BOUNDARIES_D";
	condition = condition || get<parameters::modules::pressure>() == "densityMapIISPH";
	return condition;
}
}
...
namespace renderArrays{
float4* anisotropicSupport::ptr = nullptr;
size_t anisotropicSupport::alloc_size = 0;


void anisotropicSupport::defaultAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(float4);
	cudaAllocateMemory(&ptr, alloc_size);
}
void anisotropicSupport::leanAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(float4);
	
}

void anisotropicSupport::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
}
void anisotropicSupport::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
}
anisotropicSupport::operator type*(){ return ptr;}
float4& anisotropicSupport::operator[](size_t idx){ return ptr[idx];}
bool anisotropicSupport::valid(){
	bool condition = true;
	condition = condition && get<parameters::modules::rayTracing>() == true;
	condition = condition && get<parameters::modules::anisotropicSurface>() == true;
	return condition;
...
namespace renderArrays{
uint32_t* auxLength::ptr = nullptr;
size_t auxLength::alloc_size = 0;


void auxLength::defaultAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(uint32_t);
	cudaAllocateMemory(&ptr, alloc_size);
}
void auxLength::leanAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(uint32_t);
	
}

void auxLength::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
}
void auxLength::free(){
...
auxLength::operator type*(){ return ptr;}
uint32_t& auxLength::operator[](size_t idx){ return ptr[idx];}
bool auxLength::valid(){
	bool condition = true;
	condition = condition && get<parameters::modules::rayTracing>() == true;
	return condition;
}
...
void compactHashMap::defaultAllocate(){
	auto elems = (1);
	alloc_size = get<parameters::simulation_settings::mlm_schemes>() * get<parameters::simulation_settings::hash_entries>() * elems * sizeof(compactHashSpan);
	cudaAllocateMemory(&ptr, alloc_size);
}
...
void hashMap::defaultAllocate(){
	auto elems = (1);
	alloc_size = get<parameters::simulation_settings::mlm_schemes>() * get<parameters::simulation_settings::hash_entries>() * elems * sizeof(hash_span);
	cudaAllocateMemory(&ptr, alloc_size);
}
...
void compactCellSpan::defaultAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = get<parameters::simulation_settings::mlm_schemes>() * elems * sizeof(compact_cellSpan);
	cudaAllocateMemory(&ptr, alloc_size);
}
...
void cellSpan::defaultAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = get<parameters::simulation_settings::mlm_schemes>() * elems * sizeof(cell_span);
	cudaAllocateMemory(&ptr, alloc_size);
}
...
void neighborList::defaultAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = get<parameters::simulation_settings::neighborlimit>() * elems * sizeof(int32_t);
	cudaAllocateMemory(&ptr, alloc_size);
}
...
namespace structureArrays{
int32_t* refinementList::ptr = nullptr;
size_t refinementList::alloc_size = 0;


void refinementList::defaultAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = get<parameters::simulation_settings::neighborlimit>() * elems * sizeof(int32_t);
	cudaAllocateMemory(&ptr, alloc_size);
}
void refinementList::leanAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = get<parameters::simulation_settings::neighborlimit>() * elems * sizeof(int32_t);
	
}

void refinementList::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
}
void refinementList::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
}
refinementList::operator type*(){ return ptr;}
int32_t& refinementList::operator[](size_t idx){ return ptr[idx];}
bool refinementList::valid(){
	bool condition = true;
	return condition;
}
}
...
namespace structureArrays{
int32_t* refinementListLength::ptr = nullptr;
size_t refinementListLength::alloc_size = 0;

int32_t* refinementListLength::rear_ptr = nullptr;
void refinementListLength::swap() { std::swap(ptr, rear_ptr); }

void refinementListLength::defaultAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(int32_t);
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void refinementListLength::leanAllocate(){
	auto elems = (get<parameters::simulation_settings::maxNumptcls>() + 1);
	alloc_size = 1 * elems * sizeof(int32_t);
...
void refinementListLength::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
	cudaAllocateMemory(&rear_ptr, alloc_size);
}
void refinementListLength::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
	cudaFree(rear_ptr);
...
refinementListLength::operator type*(){ return ptr;}
int32_t& refinementListLength::operator[](size_t idx){ return ptr[idx];}
bool refinementListLength::valid(){
	bool condition = true;
	return condition;
}
}
...
void cellBegin::defaultAllocate(){
	auto elems = get<parameters::internal::gridSize>().x * get<parameters::internal::gridSize>().y * get<parameters::internal::gridSize>().z;
	alloc_size = 1 * elems * sizeof(int32_t);
	cudaAllocateMemory(&ptr, alloc_size);
}
...
namespace volumeInletArrays{
float4* volumeOutletVoxelSizes::ptr = nullptr;
size_t volumeOutletVoxelSizes::alloc_size = 0;



void volumeOutletVoxelSizes::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
}
void volumeOutletVoxelSizes::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
}
volumeOutletVoxelSizes::operator type*(){ return ptr;}
float4& volumeOutletVoxelSizes::operator[](size_t idx){ return ptr[idx];}
bool volumeOutletVoxelSizes::valid(){
	bool condition = true;
	return condition;
}
}
namespace volumeInletArrays{
float4* volumeOutletOffsets::ptr = nullptr;
size_t volumeOutletOffsets::alloc_size = 0;



void volumeOutletOffsets::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
}
void volumeOutletOffsets::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
}
volumeOutletOffsets::operator type*(){ return ptr;}
float4& volumeOutletOffsets::operator[](size_t idx){ return ptr[idx];}
bool volumeOutletOffsets::valid(){
	bool condition = true;
	return condition;
}
}
...
namespace volumeInletArrays{
gvdb::VDBInfo** outletGVDBVolumes::ptr = nullptr;
size_t outletGVDBVolumes::alloc_size = 0;



void outletGVDBVolumes::allocate(size_t size){
	alloc_size = size;
	cudaAllocateMemory(&ptr, alloc_size);
}
void outletGVDBVolumes::free(){
	alloc_size = 0;
	cudaFree(ptr);
	ptr = nullptr;
}
outletGVDBVolumes::operator type*(){ return ptr;}
gvdb::VDBInfo*& outletGVDBVolumes::operator[](size_t idx){ return ptr[idx];}
bool outletGVDBVolumes::valid(){
	bool condition = true;
	return condition;
}
}
...
std::tuple<arrays::boundaryArrays::volumeBoundaryVolumes, arrays::boundaryArrays::volumeBoundaryGVDBVolumes, arrays::boundaryArrays::gvdbOffsets, arrays::boundaryArrays::gvdbVoxelSizes, arrays::boundaryArrays::volumeBoundaryDimensions, arrays::boundaryArrays::volumeBoundaryMin, arrays::boundaryArrays::volumeBoundaryMax, arrays::boundaryArrays::volumeBoundaryDensity, arrays::boundaryArrays::volumeBoundaryVolume, arrays::boundaryArrays::volumeBoundaryVelocity, arrays::boundaryArrays::volumeBoundaryAngularVelocity, arrays::boundaryArrays::volumeBoundaryKind, arrays::boundaryArrays::volumeBoundaryPosition, arrays::boundaryArrays::volumeBoundaryQuaternion, arrays::boundaryArrays::volumeBoundaryTransformMatrix, arrays::boundaryArrays::volumeBoundaryTransformMatrixInverse, arrays::boundaryArrays::volumeBoundaryInertiaMatrix, arrays::boundaryArrays::volumeBoundaryInertiaMatrixInverse, arrays::boundaryArrays::volumeBoundaryAcceleration, arrays::boundaryArrays::volumeBoundaryAngularAcceleration, arrays::basicArrays::matrixTest, arrays::basicArrays::sdf_gridsize, arrays::basicArrays::sdf_minpoint2, arrays::basicArrays::sdf_minpoint1> individual_list;
std::tuple<arrays::adaptive::splitIndicator, arrays::adaptive::mergeIndicator, arrays::adaptive::parentIndex, arrays::adaptive::parentVolume, arrays::adaptive::parentPosition, arrays::advectionArrays::angularVelocity, arrays::advectionArrays::vorticity, arrays::surfaceArrays::distanceBuffer, arrays::basicArrays::position, arrays::basicArrays::acceleration, arrays::basicArrays::acceleration_inter, arrays::basicArrays::velocity, arrays::basicArrays::rigidbody_sdf2, arrays::basicArrays::particle_type, arrays::basicArrays::tobeoptimized, arrays::basicArrays::uid, arrays::basicArrays::renderArray, arrays::basicArrays::debugArray, arrays::basicArrays::volume, arrays::basicArrays::lifetime, arrays::basicArrays::optimization_lifetime, arrays::basicArrays::optimization_lifetime_merge, arrays::basicArrays::optimization_group, arrays::basicArrays::pressure, arrays::basicArrays::density, arrays::basicArrays::old_density, arrays::basicArrays::density_b, arrays::basicArrays::density_f, arrays::structureArrays::neighborListLength, arrays::structureArrays::refinementListLength> sorting_list;
