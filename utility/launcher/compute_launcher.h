...
struct computeLauncher {
	...
	template <typename T> hostOnly void host_launch(int32_t elements, Vs... args) {
		if (m_timeCode && m_host_timer == nullptr)
			m_host_timer = TimerManager::createTimer(m_name, m_col, "",0);
		cuda::sync();
	...
	template <typename T> hostOnly void device_launch(int32_t elements, Vs... args) {
		if (m_timeCode && m_device_timer == nullptr)
			m_device_timer = TimerManager::createGPUTimer(m_name, m_col, "",0);
	...
	template <typename T> hostOnly void debug_launch(int32_t elements, Vs... args) {
		...
		if (m_host_timer != nullptr) {
			...
			std::cout << "Function " << m_name << " finished after: " << last << "ms."
				<< std::endl;
		}
		else {
	...
	template <typename T> struct Delegator<launch_config::_used_for_template_specializations, T> {
		template <typename L, typename Ty, typename... Us>
		static void call(L *launcher, Ty elements, Us &&... args) {
			launch_config target = get<parameters::internal::target>();
			if (get<parameters::modules::regex_cfg>()) {
				...
				matcher(get<parameters::simulation_settings::deviceRegex>(), launch_config::device);
				matcher(get<parameters::simulation_settings::hostRegex>(), launch_config::host);
				matcher(get<parameters::simulation_settings::debugRegex>(), launch_config::debug);
			}
