...
#define CONSTRAINED_NEIGHBORS
#define COMPACTMLM_CELLS
#if defined(CONSTRAINED_NEIGHBORS)
//#define iterateNeighbors(var) for (const auto& var: neighbor_span<neighbor_list::constrained>(i, arrays))
#define iterateNeighbors(var) for(int32_t jj = 0, j = arrays.neighborList[i]; jj < arrays.neighborListLength[i]; (++jj), (j = arrays.neighborList[i + jj * arrays.maxNumptcls]))
#elif defined(CELLBASED_NEIGHBORS)
#define iterateNeighbors(var) for (const auto& var: neighbor_span<neighbor_list::cell_based>(i, arrays))
#elif defined(BASIC_NEIGHBORS)
#define iterateNeighbors(var) for (const auto& var: neighbor_span<neighbor_list::constrained>(i, arrays))
#elif defined(COMPACTCELL_NEIGHBORS)
#define iterateNeighbors(var) for (const auto& var: neighbor_span<neighbor_list::compactCell>(i, arrays))
#elif defined(COMPACTMLM_NEIGHBORS)
#define iterateNeighbors(var) for (const auto& var: cell_iterate<hash_length::bit_64, cell_ordering::z_order, cell_structuring::compactMLM>(pairHelper(memory.position)[idx], memory))
#elif defined(MASKED_NEIGHBORS)
#define iterateNeighbors(var) for (const auto& var: neighbor_span<neighbor_list::masked>(i, arrays))
#endif
//#define iterateNeighbors(var) for (const auto& var : neighbor_iterate_wrapper(i, arrays, false, neigh_tag_ty<neighborhood>{}))
#if defined(LINEAR32_CELLS)
#define iterateCells(position,var) for (const auto& var : cell_interpolator<hash_length::bit_32, cell_ordering::linear_order, cell_structuring::complete, std::decay_t<decltype(arrays)>>( position, arrays))
#define iterateAllCells(position,var) for (const auto& var : cell_interpolator<hash_length::bit_32, cell_ordering::linear_order, cell_structuring::complete, std::decay_t<decltype(arrays)>>( position, arrays, true))
#elif defined(ZORDER32_CELLS)
#define iterateCells(position,var) for (const auto& var : cell_interpolator<hash_length::bit_32, cell_ordering::z_order, cell_structuring::hashed, std::decay_t<decltype(arrays)>>( position, arrays))
#define iterateAllCells(position,var) for (const auto& var : cell_interpolator<hash_length::bit_32, cell_ordering::z_order, cell_structuring::hashed, std::decay_t<decltype(arrays)>>( position, arrays, true))
#elif defined(ZORDER64_CELLS)
#define iterateCells(position,var) for (const auto& var : cell_interpolator<hash_length::bit_64, cell_ordering::z_order, cell_structuring::hashed, std::decay_t<decltype(arrays)>>( position, arrays))
#define iterateAllCells(position,var) for (const auto& var : cell_interpolator<hash_length::bit_64, cell_ordering::z_order, cell_structuring::hashed, std::decay_t<decltype(arrays)>>( position, arrays, true))
#elif defined(MLM_CELLS)
#define iterateCells(position,var) for (const auto& var : cell_interpolator<hash_length::bit_64, cell_ordering::z_order, cell_structuring::MLM, std::decay_t<decltype(arrays)>>( position, arrays))
#define iterateAllCells(position,var) for (const auto& var : cell_interpolator<hash_length::bit_64, cell_ordering::z_order, cell_structuring::MLM, std::decay_t<decltype(arrays)>>( position, arrays, true))
#elif defined(COMPACTMLM_CELLS)
#define iterateCells(position,var) for (const auto& var : cell_interpolator<hash_length::bit_64, cell_ordering::z_order, cell_structuring::compactMLM, std::decay_t<decltype(arrays)>>( position, arrays))
#define iterateAllCells(position,var) for (const auto& var : cell_interpolator<hash_length::bit_64, cell_ordering::z_order, cell_structuring::compactMLM, std::decay_t<decltype(arrays)>>( position, arrays, true))
#endif
...
__host__ __device__ void cell_linear_interpolate(Func &&fn, T &&mem, U position) {
	uint32_t num_cells = math::unit_get<1>(mem.gridSize) * math::unit_get<2>(mem.gridSize) *
		math::unit_get<3>(mem.gridSize);
		...
		for (int z = -1; z < 2; ++z) {
			...
			for (int y = -1; y < 2; ++y) {
				...
				for (int x = -1; x < 2; ++x) {
					...
					if (cellIdx.x > -1 && cellIdx.x < (int)math::unit_get<1>(mem.gridSize) &&
						cellIdx.y > -1 && cellIdx.y < (int)math::unit_get<2>(mem.gridSize) &&
						cellIdx.z > -1 && cellIdx.z < (int)math::unit_get<3>(mem.gridSize)) {
						uint idx = idx3D_to_linear(
							...
							uint3{ static_cast<uint32_t>(math::unit_get<1>(mem.gridSize)),
							static_cast<uint32_t>(math::unit_get<2>(mem.gridSize)),
							static_cast<uint32_t>(math::unit_get<3>(mem.gridSize)) });
						uint2 cellspan;
						if (idx < num_cells) {
