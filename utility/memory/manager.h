...
struct MemoryManager {
	...
	template <typename P, typename U> void deallocate( P arr, U &memory) {
		using mem_t = std::decay_t<decltype(P::get_member(memory))>;
		mem_t ptr = (mem_t)P::ptr;
		if (P::variableName == get<parameters::color_map::buffer>() || P::qualifiedName == get<parameters::color_map::buffer>())
			return;
