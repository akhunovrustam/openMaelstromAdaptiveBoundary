...
struct cell_interpolator<hash_length::bit_64, cell_ordering::z_order, cell_structuring::hashed,
	...
	struct cell_iterator {
		...
		Mem &arrays;
                hash_span s{INVALID_BEG, 0};
                cell_span cs{INVALID_BEG, 0};

		hostDevice int32_t cs_loop() {
                  if (cs.beginning != INVALID_BEG && jj < cs.beginning + cs.length) {
				neighbor_idx = jj;
				++jj;
				return neighbor_idx;
			}
...
struct cell_interpolator<hash_length::bit_32, cell_ordering::linear_order,
	...
	struct cell_iterator {
		...
		hostDevice void increment() {
			if (c_loop() != -1)
				return;
			uint32_t num_cells = math::unit_get<1>(arrays.gridSize) *
				math::unit_get<2>(arrays.gridSize) *
				math::unit_get<3>(arrays.gridSize);
			int3 cellIdx;
			for (; i <= 1; ++i) {
				...
				for (; j <= 1; ++j) {
					...
					for (; k <= 1;) {
						...
						++k;
						if (cellIdx.x > -1 && cellIdx.x < (int)math::unit_get<1>(arrays.gridSize) &&
							cellIdx.y > -1 && cellIdx.y < (int)math::unit_get<2>(arrays.gridSize) &&
							cellIdx.z > -1 && cellIdx.z < (int)math::unit_get<3>(arrays.gridSize)) {
							uint idx = idx3D_to_linear(
								...
								uint3{ static_cast<uint32_t>(math::unit_get<1>(arrays.gridSize)),
								static_cast<uint32_t>(math::unit_get<2>(arrays.gridSize)),
								static_cast<uint32_t>(math::unit_get<3>(arrays.gridSize)) });
							if (idx < num_cells) {
	...
	struct cell_iterator {
		...
		hostDevice int32_t s_loop() {
			if (s.beginning != INVALID_BEG) {
				...
				for (; ii < s.beginning + s.length;) {
					cs = arrays.cellSpan[ii + arrays.maxNumptcls * resolution];
					++ii;
					jj = cs.beginning;
					if (full_loop ||
...
struct cell_interpolator<hash_length::bit_64, cell_ordering::z_order, cell_structuring::compactMLM, Mem> {
	...
	struct cell_iterator {
		...
		compact_cellSpan cs{ 0,INVALID_BEG, 0 };
		...
		hostDevice int32_t s_loop() {
				...
				for (; ii < s.beginning + s.length;) {
					cs = arrays.compactCellSpan[ii + arrays.maxNumptcls * resolution];
					++ii;
					jj = cs.beginning;
					if (full_loop ||
						position_to_morton(pairHelper(arrays.position)[cs.beginning], arrays, factor) == morton) {
