...
template <> struct neighbor_span<neighbor_list::constrained> {
	...
	hostDeviceInline neighbor_span(const int32_t idx, const T &arrays)
		: offset(arrays.maxNumptcls), pData((const int32_t *)arrays.neighborList + idx),
...
template <> struct neighbor_span<neighbor_list::cell_based> {
	...
	hostDeviceInline neighbor_it begin() const { return neighbor_it{ pData, offset }; }
	hostDeviceInline neighbor_it end() const { return neighbor_it{ pData, offset, INVALID_BEG_VALUE }; }
	hostDeviceInline neighbor_it cbegin() const { return neighbor_it{ pData, offset }; }
	hostDeviceInline neighbor_it cend() const { return neighbor_it{ pData, offset, INVALID_BEG_VALUE }; }
};
	...
	struct iterator {
		...
		hostDeviceInline  void increment() {
			if (mIt.c.mask == MAX_VAL_26BIT) {
                                if (j >= (int32_t)mIt.cs.beginning && j + 1 < (int32_t)(mIt.cs.beginning + mIt.cs.length)) {
					++j;
					return;
				}
			}
			...
			mIt.c = mIt.arrays.neighborMask[mIt.i + mIt.ci * mIt.arrays.maxNumptcls];
			mIt.ci++;
			if (mIt.c.mask == 0) {
				j = -1;
				done = true;
			...
			auto s = mIt.arrays.compactHashMap[idx3D_to_hash(cell, mIt.arrays.hash_entries) + mIt.arrays.hash_entries * mIt.resolution];
                        mIt.cs = compact_cellSpan{0,0, 0};
			if (s.compacted) {
                          mIt.cs = compact_cellSpan{0,s.beginning, s.length};
			}
			else {
                                for (int32_t si = (int32_t)s.beginning; si < (int32_t) (s.beginning + s.length); ++si) {
					mIt.cs = mIt.arrays.compactCellSpan[si + mIt.arrays.maxNumptcls * mIt.resolution];
