...
namespace bt {
	...
	World::World() {
		...
		dynamicsWorld->setGravity(bt3fromf3(math::castTo<float3>(get<parameters::simulation_settings::external_force>())));
		///-----initialization_end-----
		{
		...
		for (int32_t b = 0; b < get<parameters::boundary_volumes::volumeBoundaryCounter>(); ++b) {
			auto boundaryVolume = get<parameters::boundary_volumes::volume>()[b];
			auto file = resolveFile(boundaryVolume.fileName, { get<parameters::internal::config_folder>() });
			auto f_obj = file;
			...
			if (boundaryObjects.find(f_obj.string()) != boundaryObjects.end()) {
				...
				robj = generation::fileToObj(std::filesystem::exists(f_obj) ? f_obj : file);
			...
			if (volumeObjects.find(file.string()) != volumeObjects.end()) {
				...
				vobj = generation::cudaVolume(boundaryVolume.fileName);
			...
			auto mass = V * boundaryVolume.density;
			...
			startTransform.setOrigin(bt3fromf3(boundaryVolume.position + math::castTo<float3>(centerOfMass)));
			auto angle = boundaryVolume.angle;
			angle = angle / 180.f * CUDART_PI_F;
			startTransform.setRotation(btqfromf4(eul2quat(angle)));

			bool isDynamic = (mass != 0.f);
			if (boundaryVolume.kind != 0) {
				isDynamic = false;
				mass = 0.f;
			}
	...
	void World::step(float dt) {
		dynamicsWorld->stepSimulation(dt, get<parameters::simulation_settings::timestep_max>() / get<parameters::simulation_settings::timestep_min>() * 4.f, get<parameters::simulation_settings::timestep_min>()*0.25f);
	}
