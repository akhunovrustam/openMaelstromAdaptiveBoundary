...
#define FILE_FORMAT_VERSION 0x10011
...
std::pair<float3, float3> getDomain() {
  return std::make_pair(get<parameters::internal::min_domain>(), get<parameters::internal::max_domain>());
}
...
std::tuple<cudaTextureObject_t, float4, float4, int4, float4, Matrix4x4> cudaVolume(std::string fileName) {
  ...
  if (tmpFileExists) {
    ...
    if (marker != 0)
      ...
      if (marker != FILE_FORMAT_VERSION)
        ...
        //processed_vdb.read(reinterpret_cast<char *>(host_mem),
        //                   dimension.x * dimension.y * dimension.z * sizeof(float4));
  ...
  if (!readFromTmp) {
    ...
    normalSet->setTransform(vdbm::Transform::createLinearTransform(levelSet->voxelSize().x()));
    ...
    vdbm::UniformScaleMap map(levelSet->voxelSize().x());
    ...
    auto transformer = [&, levelSet = levelSet](auto iter, auto accessor) {
      ...
      vdbm::Vec3d wsGrad =
          vdbm::Gradient<vdbm::UniformScaleMap, vdbm::CD_6TH>::result(map, c_accessor, iter.getCoord());
    ...
    min = levelSet->indexToWorld(box.getStart());//    -openvdb::Vec3d(0.5, 0.5, 0.5);
    max = levelSet->indexToWorld(box.getEnd());   // + openvdb::Vec3d(0.5, 0.5, 0.5);
    ...
    auto getWs = [&](uint32_t x, uint32_t y, uint32_t z) {
      ...
      vdb::Vec3d offs{((double)x) / ((double)(dimension.x-1)), ((double)y) / ((double)(dimension.y-1)),
                      ((double)z) / ((double)(dimension.z-1))};
      return pos + diff * offs;
    };
    auto dh = levelSet->voxelSize().x()*0.5f;
    auto getSample = [&](uint32_t x, uint32_t y, uint32_t z) {
      auto ws = getWs(x, y, z);
      auto d = d_sampler.wsSample(ws);
      auto dxp = d_sampler.wsSample(ws + openvdb::Vec3d(dh, 0., 0.));
      auto dxm = d_sampler.wsSample(ws - openvdb::Vec3d(dh, 0., 0.));
      auto dyp = d_sampler.wsSample(ws + openvdb::Vec3d(0., dh, 0.));
      auto dym = d_sampler.wsSample(ws - openvdb::Vec3d(0., dh, 0.));
      auto dzp = d_sampler.wsSample(ws + openvdb::Vec3d(0., 0., dh));
      auto dzm = d_sampler.wsSample(ws - openvdb::Vec3d(0., 0., dh));
      //openvdb::Vec3d n = openvdb::Vec3d((dxp - dxm) / 2.f, (dyp - dym) / 2.f, (dzp - dzm) / 2.f);
      auto n = n_sampler.wsSample(ws);
      n.normalize();
      return float4{(float)n.x(), (float)n.y(), (float)n.z(), d};
    };
    vdb::Vec3d diff = max - min;
    float3 dX{(float)diff.x() / (float)dimension.x, (float)diff.y() / (float)dimension.y,
              (float)diff.z() / (float)dimension.z};
    ...
    //std::cout << "Sample at " << openvdb::Vec3d(  0,   0,   0) << " -> " << d_sampler.wsSample(openvdb::Vec3d(0, 0, 0)) << std::endl;
    //std::cout << "Sample at " << openvdb::Vec3d( 10,   0,   0) << " -> " << d_sampler.wsSample(openvdb::Vec3d(10, 0, 0)) << std::endl;
    //std::cout << "Sample at " << openvdb::Vec3d(  0,  10,   0) << " -> " << d_sampler.wsSample(openvdb::Vec3d(0, 10, 0)) << std::endl;
    //std::cout << "Sample at " << openvdb::Vec3d(  0,   0,  10) << " -> " << d_sampler.wsSample(openvdb::Vec3d(0, 0, 10)) << std::endl;
    //std::cout << "Sample at " << openvdb::Vec3d(-10,   0,   0) << " -> " << d_sampler.wsSample(openvdb::Vec3d(-10, 0, 0)) << std::endl;
    //std::cout << "Sample at " << openvdb::Vec3d(  0, -10,   0) << " -> " << d_sampler.wsSample(openvdb::Vec3d(0, -10, 0)) << std::endl;
    //std::cout << "Sample at " << openvdb::Vec3d(  0,   0, -10) << " -> " << d_sampler.wsSample(openvdb::Vec3d(0, 0, -10)) << std::endl;

    //std::cout << "Sample at [0 0 0] : " << getSample(124 + 0, 124 + 0, 124 + 0) << std::endl;
    //std::cout << "Sample at [0 0 1] : " << getSample(124 + 0, 124 + 0, 124 + 1) << std::endl;
    //std::cout << "Sample at [0 1 0] : " << getSample(124 + 0, 124 + 1, 124 + 0) << std::endl;
    //std::cout << "Sample at [0 1 1] : " << getSample(124 + 0, 124 + 1, 124 + 1) << std::endl;
    //std::cout << "Sample at [1 0 0] : " << getSample(124 + 1, 124 + 0, 124 + 0) << std::endl;
    //std::cout << "Sample at [1 0 1] : " << getSample(124 + 1, 124 + 0, 124 + 1) << std::endl;
    //std::cout << "Sample at [1 1 0] : " << getSample(124 + 1, 124 + 1, 124 + 0) << std::endl;
    //std::cout << "Sample at [1 1 1] : " << getSample(124 + 1, 124 + 1, 124 + 1) << std::endl;

    //std::cout << "Sample at [0 0 0] : " << getSample(125 + 0, 125 + 0, 125 + 0) << std::endl;
    //std::cout << "Sample at [0 0 1] : " << getSample(125 + 0, 125 + 0, 125 + 1) << std::endl;
    //std::cout << "Sample at [0 1 0] : " << getSample(125 + 0, 125 + 1, 125 + 0) << std::endl;
    //std::cout << "Sample at [0 1 1] : " << getSample(125 + 0, 125 + 1, 125 + 1) << std::endl;
    //std::cout << "Sample at [1 0 0] : " << getSample(125 + 1, 125 + 0, 125 + 0) << std::endl;
    //std::cout << "Sample at [1 0 1] : " << getSample(125 + 1, 125 + 0, 125 + 1) << std::endl;
    //std::cout << "Sample at [1 1 0] : " << getSample(125 + 1, 125 + 1, 125 + 0) << std::endl;
    //std::cout << "Sample at [1 1 1] : " << getSample(125 + 1, 125 + 1, 125 + 1) << std::endl;
    ...
    sumPosition /= samples;
    //centerOfMass = float4{ 0.f,0.f,0.f,0.f };
    ...
    processed_vdb.write(reinterpret_cast<char *>(&centerOfMass), sizeof(float4));
  ...
  auto min_f =
      float4{static_cast<float>(min.x()), static_cast<float>(min.y()), static_cast<float>(min.z()), 0.f} - centerOfMass;
  auto max_f =
      float4{static_cast<float>(max.x()), static_cast<float>(max.y()), static_cast<float>(max.z()), 0.f} - centerOfMass;
  //free(host_mem);
  // std::cout << fileName << ": " << std::endl;
