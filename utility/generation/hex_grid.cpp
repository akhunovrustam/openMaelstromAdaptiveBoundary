...
std::vector<vdb::Vec4f> VolumeToHex(vdb::FloatGrid::Ptr grid, vdb::Vec3d min_vdb, vdb::Vec3d max_vdb, float r, float3 scale) {
  ...
  do {
    do { 
      do {
        ...
        if (sampler.wsSample(vdb::Vec3d(inserted.x() / scale.x, inserted.y() / scale.y, inserted.z() / scale.z)) < -spacing * 1.25f)
          out_points.push_back(inserted);
      } while (inserted.x() / scale.x < max_vdb.x() - 0.f * spacing);
      i = 0;
      ++j;
    } while (inserted.y() / scale.y < max_vdb.y() - 0.f * spacing);
    j = 0;
    k++;
  } while (inserted.z() / scale.z < max_vdb.z() - 0.f * spacing);
