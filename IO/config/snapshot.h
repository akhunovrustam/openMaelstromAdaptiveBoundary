...
struct UniformSnap : public SnapShot {
    YAML::Node config;

    virtual void save() override { 
        //YAML::hexFloatConversion = true;
        config = Clone(ParameterManager::instance().buildTree());
        //YAML::hexFloatConversion = false;
        YAML::Emitter out;
        out << config;
        std::clog << out.c_str() << std::endl;
    }
  virtual void load() override {
     //YAML::hexFloatConversion = true;
     ParameterManager::instance().loadTree(config);
     //YAML::hexFloatConversion = false;
  }
