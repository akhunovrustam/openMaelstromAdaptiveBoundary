  ...
  if constexpr (std::is_same<T, std::string>::value)
    ...
    struct csv_reader : std::ctype<char> {
      ...
      static std::ctype_base::mask const *get_table() {
        static std::vector<std::ctype_base::mask> rc(std::ctype_base::table_size, std::ctype_base::mask());
...
template <typename T> std::string convertToString( T argument, std::string = "") {
  setlocale(LC_ALL, "C");
  if constexpr (std::is_same<T, std::string>::value)
    return argument;
  else if constexpr (std::is_same<T, bool>::value) {
