...
void IO::config::show_config() {
	...
	YAML::Emitter out;
	out << ParameterManager::instance().buildTree();
    logger(log_level::info) << out.c_str() << std::endl;
...
void IO::config::load_config(std::string filepath) {
  ...
  ParameterManager::instance().load(filepath);
  //std::stringstream ss;
  ...
  if (arguments::cmd::instance().jsons.size() > 0) {
      std::stringstream sstream;
      for (auto j : arguments::cmd::instance().jsons) {
          auto id = j.substr(0, j.find_first_of("="));
          auto param = j.substr(j.find_first_of("=") + 1);
          auto ns = id.substr(0, id.find("."));
          id = id.substr(id.find(".") + 1);
          sstream << ns << ":\n    " << id << ": " << param << "\n";
          //std::cout << ns << " :: " << id << " -> " << param << std::endl;

          //throw std::exception("Additional parameters not currently supported.");
      ...
      YAML::Node n = YAML::Load(sstream.str().c_str());
      ParameterManager::instance().loadTree(n);
  }

  std::ifstream iss;
  std::vector<std::tuple<std::string, std::size_t, void*>> arrs;
  if(arguments::cmd::instance().snapped){
      auto path = arguments::cmd::instance().snapFile;
      path = path.substr(0, path.find_last_of("."));
      path = path + ".yaml";
      ParameterManager::instance().load(path);
      defaultAllocate();
      //throw std::exception("Snapshots not supported currently.");
 ...
 // std::cout << "Done reading dump file" << std::endl;
  ...
  get<parameters::internal::config_file>() = filepath;
  get<parameters::internal::config_folder>() = folder;

  if (get<parameters::simulation_settings::maxNumptcls>() == 0)
    get<parameters::simulation_settings::maxNumptcls>() = get<parameters::internal::num_ptcls>();
