...
void particle::loadParticles() {
	for (auto sets : get<parameters::particleSets::set>()) {
		auto& numPtcls = get<parameters::internal::num_ptcls>();
		auto file = resolveFile(sets, { get<parameters::internal::config_folder>() });
		auto ptcls = 0;
...
void particle::saveParticles() {
	...
	auto ptcls = get<parameters::internal::num_ptcls>();
