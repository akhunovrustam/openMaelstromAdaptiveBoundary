...
void SPH::distance::distance(Memory mem) {
  ...
  // TODO: FIXME!
    std::cout << "555555555555555555!! phiorig \n";
  int32_t diff = 0;
  auto compact_idx = [&]() {
  ...
  {
      if (phimin == 0.123f || phimin < mem.distanceBuffer.first[i].val)
          phimin = mem.distanceBuffer.first[i].val;
  }
  std::cout << "555555555555555555!! phiorig " << uGet<parameters::surfaceDistance::surface_phiMin>().val << " phimine " << phimin << "\n";
  //if (get<parameters::adaptive::ratio>() > 1.f)
	 // get<parameters::surface_neighborLimit>() = 25;
}
