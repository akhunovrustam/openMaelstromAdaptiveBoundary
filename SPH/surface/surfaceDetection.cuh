...
namespace SPH{
	namespace detection{
		struct Memory{
			...
			using input_arrays = std::tuple<arrays::position, arrays::density, arrays::volume, arrays::velocity, arrays::particle_type, arrays::uid>;
...
constexpr static const bool inlet = false;
		...
		inline void setupParameters(Memory& arrays){
			...
			arrays.first_fluid = uGet<parameters::particle_settings::first_fluid>();

		}
#endif
		
		void distance(Memory mem = Memory());
		void fluidneighbor(Memory mem = Memory());
		void showBuffer(Memory mem = Memory());
	} // namspace detection
}// namespace SPH
