	...
	namespace compactMLM {
		...
		basicFunctionType cudaCellTablea(SPH::compactMLM::Memory arrays, int32_t threads, int32_t *compact, float) {
			checkedThreadIdx(i);
                  arrays.compactCellSpan[i] =
                      compact_cellSpan{0, (uint32_t)(compact[i]), (uint32_t)(compact[i + 1] - compact[i])};
		}
		...
		basicFunctionType compactHashMap(SPH::compactMLM::Memory arrays, int32_t threads) {
			...
			if (hashEntry.length == 1) 
				arrays.compactHashMap[h] = compactHashSpan{ 1, cell.beginning, cell.length };
			else
		...
		struct count_if {
                  hostDeviceInline bool operator()(const compactHashSpan x) {
		...
		struct is_refined{
			hostDeviceInline bool operator()(int x) { return x == 5; }
		};
		struct is_boundary {
			hostDeviceInline bool operator()(int x) { return x == 1; }
		};
		struct invalid_volume {
			hostDeviceInline bool operator()(float x) { return x == FLT_MAX || x == 0.f || x !=x; }
		};
		struct hash_spans {
                  hostDeviceInline compactHashSpan operator()() { return compactHashSpan{0, INVALID_BEG, 0}; }
		};
	} // namespace resort_mlm
} // namespace SPH

void SPH::compactMLM::markBegin(Memory mem) {
	std::cout << "-------------------------------------------------begin----------------------------------\n";
}

void SPH::compactMLM::calculateFluid(Memory mem) {
	...
	for (int i = 0; i < mem.num_ptcls; i++)
	{
		...
		return;
		//float area = M_PI * math::pow(3 * mem.volume)
	}
}
...
void SPH::compactMLM::resortParticles(Memory mem) {
	if (mem.num_ptcls > 0) {
		...
		// as this can improve re-sorting performance slightly.get<parameters::internal::cell_size>() =
          //get<parameters::internal::cell_size>() = float3{get<parameters::internal::ptcl_support>(), get<parameters::internal::ptcl_support>(),
          //                                      get<parameters::internal::ptcl_support>()} *
          //                               kernelSize() * 2.f;
          //      mem.cell_size = get<parameters::internal::cell_size>();
		auto min_coord = math::min(get<parameters::internal::min_domain>(),math::to<float3>(algorithm::reduce_min(arrays::position::ptr, mem.num_ptcls)));
		...
		for (int32_t i = 0; i < (int32_t)get<parameters::simulation_settings::mlm_schemes>(); ++i) {
			...
			compactHashSpan *hashMap = arrays::compactHashMap::ptr + get<parameters::simulation_settings::hash_entries>() *i;
			compact_cellSpan *cellSpan = arrays::compactCellSpan::ptr + get<parameters::simulation_settings::maxNumptcls>() *i;
			...
			cuda::sync();
			//int32_t collisionsg = algorithm::count_if((mem.hashMap), get<parameters::simulation_settings::hash_entries>(), count_if());
			//get<parameters::resort::collision_cells>() += collisionsg;
			//if(i == 0)
