...
namespace SPH{
	namespace compactMLM{
		struct Memory{
			...
			const_array<arrays::optimization_group> optimization_group;
			const_array<arrays::uid> uid;
			...
			using input_arrays = std::tuple<arrays::position, arrays::optimization_group, arrays::uid>;
...
constexpr static const bool inlet = false;
		...
		inline void setupParameters(Memory& arrays){
			...
			arrays.first_fluid = get<parameters::particle_settings::first_fluid>();

		}
#endif
		
		void resortParticles(Memory mem = Memory());
		void calculateFluid(Memory mem = Memory());
		void markBegin(Memory mem = Memory());
	} // namspace compactMLM
}// namespace SPH
