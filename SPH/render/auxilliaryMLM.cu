...
namespace SPH {
	...
	namespace auxilliaryMLM {
		...
		cellFunctionType cuCompactCellTable(SPH::auxilliaryMLM::Memory arrays, int32_t threads, int32_t *compact, float ratio) {
			...
			if (s.compacted == 1) {
				cs.beginning = s.beginning;
				cs.length = s.length;
			}
			if (s.compacted == 0) {
				for (auto ii = s.beginning; ii < s.beginning + s.length; ++ii) {
			...
			for (auto jj = cs.beginning; jj < cs.beginning + cs.length; ++jj) {
			...
			if (arrays.pruneVoxel == 0) pred = LIST_COMPACT;
		...
		basicFunctionDeviceType spreadCells(SPH::auxilliaryMLM::Memory arrays, int32_t threads) {
			...
			if (max_flag < 0.5f /*|| min_neighs < arrays.vrtxNeighborLimit*/ && !(arrays.pruneVoxel == 1)) return;
				...
				for (int32_t j = -1; j <= 1; ++j) {
					for (int32_t k = -1; k <= 1; ++k) {
						...
						for (int32_t ii = cs.beginning; ii < cs.beginning + cs.length; ++ii) {
							...
							//if (flag < 0.5f && !(arrays.pruneVoxel == 1)) continue;
							if (
								//doesCubeIntersectSphere(min, max, math::castTo<float3>(x_ii), support_from_volume(PI4O3 * arrays.radius * arrays.radius*arrays.radius) * kernelSize())
								true
								) {
...
void SPH::auxilliaryMLM::generateAuxilliaryGrid(Memory mem) {
	...
	if (mem.num_ptcls > 0) {
		...
		//launch<levelSetTesting>(mem.num_ptcls, mem, get<parameters::render_settings::vrtxWMin>(), get<parameters::render_settings::vrtxWMax>());
		...
		float factor = get<parameters::render_settings::auxScale>();
		launch<hashParticles>(mem.num_ptcls, mem, factor, 1.f);
		get<parameters::render_settings::auxCellCount>() = 0;
		...
		auto l0_cells = get<parameters::resort::occupiedCells>()[0];
		launchDevice<spread>(l0_cells, mem, l0_cells);
		int32_t collisionsg = (int32_t)thrust::count_if(thrust::device, thrust_ptr(mem.auxHashMap), thrust_ptr(mem.auxHashMap) + get<parameters::simulation_settings::hash_entries>(), count_if());
		get<parameters::resort::auxCollisions>() = collisionsg;
