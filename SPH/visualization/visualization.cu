...
hostDeviceInline float length3(T arg) {
    if constexpr (math::dimension<T>::value == 0)
        return static_cast<float>(arg);
    else if constexpr (math::dimension<T>::value == 1)
        return static_cast<float>(arg.x);
    else if constexpr (math::dimension<T>::value == 2)
        return static_cast<float>(std::sqrt(arg.x * arg.x + arg.y * arg.y));
    else if constexpr (math::dimension<T>::value == 3)
        return math::length3(math::castTo<float3>(arg));
    else if constexpr (math::dimension<T>::value == 4)
        return math::length3(math::castTo<float4>(arg));
    return 1.f;
}
...
void SPH::Visualizer::visualizeParticles(Memory mem) {
  if (mem.num_ptcls == 0)
    return;
  bool found = false;
  bool dotted = get<parameters::color_map::buffer>().find(".") != std::string::npos;
  //std::cout << dotted << std::endl;
  iterateArraysList([&](auto val) {
    using T = decltype(val);
    //auto variableName = T::variableName;
	//std::cout << T::variableName << " : " << T::qualifiedName << " <-> " << get<parameters::color_map::buffer>() << " -> " << ((!dotted && T::variableName == get<parameters::color_map::buffer>()) || (dotted && T::qualifiedName == get<parameters::color_map::buffer>()) )<< std::endl;
    if ((!dotted && T::variableName == get<parameters::color_map::buffer>()) || (dotted && T::qualifiedName == get<parameters::color_map::buffer>())) {
	//if (T::variableName == get<parameters::color_map::buffer>()) {
      using info = T;
	  ...
	  if(get<parameters::color_map::vectorMode>() == "length")
		  launch<transformLength3>(mem.num_ptcls, mem, T::ptr);
	  if (get<parameters::color_map::vectorMode>() == "x")
		  launch<transformX>(mem.num_ptcls, mem, T::ptr);
	  if (get<parameters::color_map::vectorMode>() == "y")
		  launch<transformY>(mem.num_ptcls, mem, T::ptr);
	  if (get<parameters::color_map::vectorMode>() == "z")
		  launch<transformZ>(mem.num_ptcls, mem, T::ptr);
	  if (get<parameters::color_map::vectorMode>() == "w")
		  launch<transformW>(mem.num_ptcls, mem, T::ptr);
     // launch<transformLength3>(mem.num_ptcls, mem, T::ptr);
      if (get<parameters::color_map::autoScaling>()) {
		  auto min = algorithm::reduce_min(mem.renderArray, mem.num_ptcls);
		  auto max = algorithm::reduce_max(mem.renderArray, mem.num_ptcls);
		  get<parameters::color_map::min>() = min.w;
		  get<parameters::color_map::max>() = max.w;
          if (get<parameters::color_map::buffer>().find("volume") != std::string::npos) {
              //std::cout << "found volume map" << std::endl;
              //std::cout << get<parameters::color_map::min>() << " - " << get<parameters::color_map::max>() << " -> \n\t" <<
              //    get<parameters::color_map::max>() / get<parameters::adaptive::resolution>() << " / " << math::max(get<parameters::color_map::min>(), get<parameters::color_map::max>() / get<parameters::adaptive::resolution>()) << std::endl;
              ////if (min.w != max.w)
              get<parameters::color_map::min>() = math::max(get<parameters::color_map::min>(), get<parameters::color_map::max>() / get<parameters::adaptive::resolution>());
          }
      }
      //launch<visualize>(mem.num_ptcls, mem, get<parameters::color_map::min>(),
       //                 get<parameters::color_map::max>());
    }
  });
  if (found == false) {
