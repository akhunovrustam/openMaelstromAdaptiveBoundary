...
void SPH::IISPH17::pressure_solve(Memory mem) {
	...
	//	cuda::sync();
	//	if (iteration > 1) {
	//		uGet<parameters::iisph_settings::density_error>() = math::getValue(algorithm::reduce_sum(mem.volumeError, mem.num_ptcls) / algorithm::reduce_sum(arrays::volume::ptr, mem.num_ptcls));
	//	}
	//	test = iteration < 2;
