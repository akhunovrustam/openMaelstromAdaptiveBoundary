...
/*
Module used to implement an Implicit Incompressible SPH (IISPH) Solver modified from the updated IISPH paper. Pressure Mirroring decoupled
*/
namespace SPH{
	namespace IISPH17_PRESSURE_MIRRORING_D{
		struct Memory{
			// basic information
			int32_t num_ptcls;
			...
			write_array_u<arrays::acceleration_inter> acceleration_inter;

			// input resources (mapped as read only)
			const_array_u<arrays::density> density;
			const_array_u<arrays::density_f> density_f;
			const_array_u<arrays::density_b> density_b;
			...
			using input_arrays = std::tuple<arrays::density, arrays::density_f, arrays::density_b, arrays::volume, arrays::particle_type, arrays::rigidDensities, arrays::uid>;
			using output_arrays = std::tuple<arrays::position, arrays::acceleration, arrays::velocity, arrays::_sourceTerm>;
			using temporary_arrays = std::tuple<arrays::_apparentVolume, arrays::_Aii, arrays::_volumeError, arrays::_omega, arrays::_predictedAcceleration, arrays::acceleration_inter>;
...
constexpr static const bool inlet = false;
		...
		inline void setupParameters(Memory& arrays){
			...
			arrays.gamma = uGet<parameters::rigid_volumes::gamma>();
			arrays.beta = uGet<parameters::rigid_volumes::beta>();
			arrays.num_ptcls_fluid = uGet<parameters::internal::num_ptcls_fluid>();

		}
#endif
		
		void pressure_solve(Memory mem = Memory());
		void print_velocity(Memory mem = Memory());
		void print_begin(Memory mem = Memory());
	} // namspace IISPH17_PRESSURE_MIRRORING_D
}// namespace SPH
