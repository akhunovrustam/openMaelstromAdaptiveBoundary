...
neighFunctionType IISPH_predictAdvection2(SPH::IISPH17_BAND_RB::Memory arrays) {
  ...
  else
  {
    ...
    //auto rad3 = arrays.radius * arrays.radius * arrays.radius * 8.f;
...
void SPH::IISPH17_BAND_RB::pressure_solve(Memory mem) {
  size_t iteration = 0;
  float limit = get<parameters::iisph_settings::eta>() / 100.f;
  bool test = true;
  ...
  do {
    ...
    if (iteration > 1) {     
      uGet<parameters::iisph_settings::density_error>() = math::getValue(algorithm::reduce_sum(mem._volumeError, mem.num_ptcls) / algorithm::reduce_sum(arrays::volume::ptr, mem.num_ptcls));
    }
    test = iteration < 2;
    test = test || (get<parameters::iisph_settings::density_error>() > limit);
    test = test && (iteration < 256);
    // std::cout << iteration << " -> " << get<parameters::iisph_settings::density_error>() << std::endl;
  } while (test);
  launch<Jacobi1>(mem.num_ptcls, mem);
  launch<Update>(mem.num_ptcls, mem);
  get<parameters::iisph_settings::iterations>() = (int32_t) iteration;
}
