			...
			for (int32_t zi = -1; zi <= 1; ++zi) {
				...
				if (s.compacted) {
					...
					for (int32_t si = (int32_t)s.beginning; si < (int32_t) (s.beginning + s.length); ++si) {
						cs = arrays.compactCellSpan[si + arrays.maxNumptcls * resolution];
						if (position_to_morton(arrays.position[cs.beginning], arrays, factor) != morton)
							continue;
						found = true;
						break;
				...
				if (neighs.mask != 0x00) {
					arrays.neighborMask[i + arrays.maxNumptcls * cellCounter] = neighs;
					cellCounter++;
				}
			}
		}
...
hostDeviceInline auto iterateMasked(int32_t i, T& arrays, Func fn) {
	...
	int3 idx = pToIdx3(x_i, arrays.min_coord, arrays.cell_size.x * factor);
				for (int32_t ci = 0; ci < 27; ++ci) {
					auto c = arrays.neighborMask[i + ci * arrays.maxNumptcls];
					if (c.mask == 0) break;
				...
				for (auto j = cs.beginning; j < cs.beginning + cs.length; ++j) {
					...
					while (c.mask != 0) {
                                    auto j = cs.beginning + ffs(c.mask) - 1;
...
void SPH::compactNeighborMask::calculate_neighborlist(Memory arrays){
	cudaMemset(arrays.neighborMask, 0x00, sizeof(compactCellNeighbors) * arrays.maxNumptcls * 28);
	cuda::sync();
