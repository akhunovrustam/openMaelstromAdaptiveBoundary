...
cellFunctionType createCellNeighborList(SPH::spanNeighborList::Memory arrays) {
  ...
  neigh_span current{INVALID_BEG_VALUE, 0};
  int32_t valid_idx = 0;
  int32_t counter = 0;
  iterateCells(x_i, j) {
    ...
    if (x_ij <= h_ij) {
      ...
      if (!code_equal(x_p, x_j)) {
        if (current.beginning != INVALID_BEG_VALUE)
  ...
  if (current.beginning != INVALID_BEG_VALUE)
    arrays.spanNeighborList[i + (valid_idx++) * arrays.num_ptcls] = current;
  arrays.spanNeighborList[i + (valid_idx)*arrays.num_ptcls] = neigh_span{INVALID_BEG_VALUE, 0};
