...
cellFunctionType try_neighborhood(SPH::ConstrainedNeighborList::Memory arrays) {
  ...
  ordered_list_highest<float_u<SI::m>, int16_t> distance_list(supports, float_u < SI::m>{FLT_MAX}, blockDim.x, ELEM_NUM);
	...
	auto idx = [=](auto id) {return arrays.maxNumptcls * id + i; };
  auto overHeadSize = arrays.overhead_size;
  auto allPtcls = arrays.maxNumptcls;
...
cellFunctionType basic_neighborhood(SPH::ConstrainedNeighborList::Memory arrays) {
	...
	iterateCells(x_i, j) {
		...
		if (x_ij <= h_ij) {
			if(curNeighborIdx < capacity)
				arrays.neighborList[arrays.maxNumptcls * curNeighborIdx + i] = j;
			else
				arrays.neighborListSwap[0] = 1;
			curNeighborIdx++;
		}
...
void SPH::ConstrainedNeighborList::calculate_neighborlist(Memory mem) {
  get<parameters::support::adjusted_particles>() = 0;
  auto resetArrays = []() {
  ...
  if (get<parameters::modules::support>() == "constrained") {
	...
	if (get<parameters::internal::frame>() == 0)
		for (int jj = 0; jj < mem.num_ptcls; jj++)
			if (mem.neighborListLength[jj] > get<parameters::particle_settings::max_neighbors>())
				get<parameters::particle_settings::max_neighbors>() = mem.neighborListLength[jj];
  ...
  do {
    ...
    get<parameters::support::adjusted_particles>() += (int32_t) algorithm::copy_if(
  ...
  get<parameters::support::support_current_iteration>() = it;
