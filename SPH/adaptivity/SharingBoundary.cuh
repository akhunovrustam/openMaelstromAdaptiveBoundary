...
neighFunctionType shareDetectBoundary(SPH::adaptive::Memory arrays) {
	...
	//iterateNeighbors(j) {
	//	ctr += 1.f;
	//	vol += arrays.volume.second[j].val;
	//}
	//vol /= ctr;
	...
	//else if (pDist.val.w < support_from_volume(1.f) * kernelSize() * 1.5f)
	//	arrays.mergeable[i] = -3;
	else if (arrays.lifetime[i].val < 5.f * arrays.timestep.val)
		arrays.mergeable[i] = -3;
	//if (arrays.volume.second[i].val > vol * 1.6f)
	//	arrays.mergeable[i] = -3;
	return;
}
...
// sizes.
neighFunctionType shareGrabEvenBoundary(SPH::adaptive::Memory arrays, int32_t* randomIdx) {
	checkedParticleIdx(i);
	i = randomIdx[i];
	if (arrays.particle_type[i] == 0) return;
	...
	iterateNeighbors(j) {
		...
		if (j != i) {
			...
			if (sum_volume >= V_jt * 1.25f)
				continue;

			auto x_i = pos[i];
			auto x_j = (pos[i] * V_d / ctr + pos[j] * vol[j]) / (sum_volume);
			//auto pDist = planeBoundary::distance(x_i, V_t, arrays);
			//if (pDist.val.w < 2.f * OFFSET(V_t.val))
			//	continue;
			//for (int32_t v = 0; v < arrays.volumeBoundaryCounter; ++v)
			//	if (volumeBoundary::distance_fn(x_i, V_t, arrays, v) < 2.f * OFFSET(V_t.val))
			//		continue;
			//pDist = planeBoundary::distance(x_j, sum_volume, arrays);
			//if (pDist.val.w < 2.f * OFFSET(sum_volume.val))
			//	continue;
			//for (int32_t v = 0; v < arrays.volumeBoundaryCounter; ++v)
			//	if (volumeBoundary::distance_fn(x_j, sum_volume, arrays, v) < 2.f * OFFSET(sum_volume.val))
			//		continue;
			//if (math::distance3(x_i, x_j) < OFFSET(V_t.val) * 2.f ||
			//	math::distance3(x_i, x_j) < OFFSET(sum_volume.val) * 2.f) continue;
			if (math::distance3(pos[i], pos[j]) < support_from_volume(sum_volume) * kernelSize() * 0.3f&&
...
neighFunctionType shareGrabOddBoundary(SPH::adaptive::Memory arrays, int32_t* randomIdx) {
	checkedParticleIdx(i);
	i = randomIdx[i];
	if (arrays.particle_type[i] == 0) return;
	if (i % 2 != 0 || arrays.mergeable[i] != -1)
...
hostDeviceInline void shareValueBoundary(uint32_t, uint32_t, float_u<SI::volume>, float_u<SI::volume>) {}
...
hostDeviceInline void shareValueBoundary(uint32_t i, uint32_t j, float_u<SI::volume> m_s,
	...
	shareValueBoundary(i, j, m_s, m_p, ref...);
}


// This function shares all particles that have found sharing partners with their appropriate
// sharing partners.
neighFunctionDeviceType shareGrabbedBoundary(SPH::adaptive::Memory arrays, Ts... tup) {
  checkedParticleIdx(i);
  return;
	if (arrays.particle_type[i] == 0) return;
  ...
  iterateNeighbors(j) {
    ...
    shareValueBoundary(i, j, V_s, V_j, tup...);
...
neighFunction(detectSharingParticlesBoundary, shareDetectBoundary, "Adaptive: init (share)");
neighFunction(grabEvenSharingParticlesBoundary, shareGrabEvenBoundary, "Adaptive: find even partners (share)", caches<float4, float, float>{});
neighFunction(grabOddSharingParticlesBoundary, shareGrabOddBoundary, "Adaptive: find odd partners (share)", caches<float4, float, float>{});
neighFunctionDevice(shareParticlesBoundary, shareGrabbedBoundary, "Adaptive: sharing particles");

// Helper function to call the sharing function properly by transforming the arguments from a tuple to a variadic list
template<typename... Ts>
auto ShareGrabbedBoundary(std::tuple<Ts...>, SPH::adaptive::Memory mem) {
