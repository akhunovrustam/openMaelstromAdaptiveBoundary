...
cellFunctionType blendDensities_device(SPH::adaptive::Memory arrays, int32_t threads) {
  ...
  auto rho_p = boundary::spline(x_p, arrays.parentVolume[i], 0.5f, arrays, boundary::kind::both);
  ...
  arrays.density[i] = math::min(arrays.density[i], arrays.density[i] * (1.f - beta) + rho_p * beta);
}
...
neighFunctionType blendVelocity_device(SPH::adaptive::Memory arrays, int32_t threads) {
  ...
  iterateNeighbors(j) {
    if (arrays.parentIndex.first[i] == arrays.parentIndex.first[j]) {
	  ...
	  //printf("par vel %d %d %f %f %f\n", i, j, arrays.velocity[j].val.x, arrays.velocity[j].val.y, arrays.velocity[j].val.z);
    }
  }
  ...
  float threshold = support_from_volume(arrays.parentVolume[i]).val * kernelSize()*0.f;
  ...
  arrays.velocity[i] = arrays.velocity[i] * (1.f - beta*2.f) + v_p * beta*2.f;
}
