...
neighFunctionType mergeDetect(SPH::adaptive::Memory arrays) {
	...
	if (arrays.lifetime[i].val < 15.f * arrays.timestep.val)
		arrays.mergeable[i] = -4;
	//if (arrays.volume.second[i].val > vol * 1.1f)
	//	arrays.mergeable[i] = -4;
}
	...
	iterateNeighbors(j) {
		...
		if (j != i) {
			...
			if (sum_radius <= arrays.radius &&
				math::distance3(pos[i], pos[j]) < (support_from_volume(sum_volume) * kernelSize() * 0.3f)&&
				...
				for (int32_t v = 0; v < arrays.volumeBoundaryCounter; ++v)
					if (volumeBoundary::distance_fn((arrays.position.first[i] * vol[i] / counter + arrays.position.first[j] * vol[j]) / (vol[i] / counter + vol[j]), vol[i] / counter + vol[j], arrays, v) < OFFSET(sum_volume.val))
						continue;
				if (j % 2 == 0) {
...
neighFunctionType mergeGrabOdd(SPH::adaptive::Memory arrays, int32_t* idxArray) {
	...
	iterateNeighbors(j) {
		...
		if (j != i) {
			...
			auto sum_radius = math::power<ratio<1, 3>>(sum_volume * PI4O3_1);
			auto targetVolume = level_estimate(arrays, -arrays.distanceBuffer.first[j]);
			if (sum_volume > targetVolume * 1.4f) continue;
			//dbgVal.x++;
			//if (sum_radius <= arrays.radius) dbgVal.y++;
			if (sum_radius <= arrays.radius && 
				math::distance3(pos[i], pos[j]) < (support_from_volume(sum_volume) * kernelSize() * 0.3f) &&
...
neighFunction(detectMergingParticles, mergeDetect, "Adaptive: init (merge)");
