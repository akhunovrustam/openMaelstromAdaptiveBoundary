...
basicFunctionType cudaHashParticles(SPH::Resort::Memory arrays) {
	...
	if (h_i != FLT_MAX && math::unit_get<1>(x_i) == math::unit_get<1>(x_i)) {
		...
		arrays.resortIndex[i] = arrays.gridSize.x * arrays.gridSize.y * arrays.gridSize.z - 1;
		arrays.particleparticleIndex[i] = i;
	}
}
...
templateFunctionType cudaSort(SPH::Resort::Memory arrays, Ts... tup) {
	...
	int32_t num_cells =(int32_t)( arrays.gridSize.x * arrays.gridSize.y * arrays.gridSize.z );
...
void SPH::Resort::resortParticles(Memory mem) {
	...
	uint lastcelloffset = mem.gridSize.x * mem.gridSize.y * mem.gridSize.z - 1;
	uint end = 0;
	uint begin = 0;
	...
	if (numInvalidPtcls > 0) {
		get<parameters::internal::num_ptcls>() -= numInvalidPtcls;
	}
