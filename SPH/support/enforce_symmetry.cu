...
cellFunctionType constrainSupportBase(SPH::enforceSymmetry::Memory arrays) {
	...
	iterateCells(x_i, j) {
		...
		auto constrained = x_ij * 2.f - h_j;
		auto actual = constrained - lower_edge * h_i;
		int32_t bin = static_cast<int32_t>(math::floorf(actual / bin_size).val);
		if (bin <= 0) bin = 0;
		if (bin >= 31) bin = 31;
		//bin = math::clamp(bin, 0, 31);
		histogram[IDX(bin)]++;
...
cellFunctionType constrainSupportCompact(SPH::enforceSymmetry::Memory arrays, int32_t threads) {
	...
	iterateCells(x_i, j) {
		...
		auto constrained = x_ij * 2.f - h_j;
		uFloat<SI::m> actual = constrained - lower_edge * h_i;
		int32_t bin = static_cast<int32_t>(math::floorf(actual / bin_size).val);
		bin = math::clamp(bin, 0, 31);
		histogram[IDX(bin)]++;
	}
...
void SPH::enforceSymmetry::constrain_support(Memory mem) {
	...
	if (get<parameters::modules::support>() != "constrained") {
		//if (get<parameters::adaptive>() != true) return;
		if (get<parameters::internal::cell_structure>() == cell_structuring::MLM
	...
	if (get<parameters::internal::cell_structure>() == cell_structuring::MLM
		...
		launch<constrain>(mem.num_ptcls, mem);
		//get<parameters::support::adjusted_particles>() = algorithm::copy_if(mem.particleIndex, mem.particleIndexCompact, mem.num_ptcls, is_valid());
		//launch <constrainCompact>(get<parameters::support::adjusted_particles::ptr, mem, *parameters::support::adjusted_particles::ptr);
		//std::cout << "Adjusted particle: " << *parameters::support::adjusted_particles>() << std::endl;
	}
