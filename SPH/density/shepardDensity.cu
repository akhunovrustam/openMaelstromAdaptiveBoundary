...
void SPH::shepardDensity::estimate_density(Memory mem) {
	...
	std::cout << get<parameters::internal::frame>() << std::endl;
	if(get<parameters::internal::frame>() < 10)
		launch<estimateDensity>(mem.num_ptcls, mem);
	else{
		if(get<parameters::internal::frame>() % get<parameters::simulation_settings::densitySteps>() == 0)
			launch<estimateDensity>(mem.num_ptcls, mem);
