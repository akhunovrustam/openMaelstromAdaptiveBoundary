...
void SPH::streamInlet::init(Memory) {
  ...
  for (auto &fluidVolume : get<parameters::inlet_volumes::volume>()) {
    std::string vdbFile = get<parameters::internal::config_folder>() + fluidVolume.fileName;
    openvdb::initialize();
    ...
    fluidVolume.inlet_radius = radius.val;
 ...
 //   
	...
	//std::cout << "Particle spacing: " << spacing.val << std::endl;
	//std::cout << "Particle volume: " << particleVolume.val << std::endl;
	//std::cout << "Particle h: " << H.val << std::endl;
   ...
   // for (auto p : out_points)
	...
	//auto r = get<parameters::particle_settings::radius>() * 1.f / powf(get<parameters::adaptive::resolution>() / 8.f,1.f/3.f);
	auto r = radius.val;
	auto volume = PI4O3 * math::power<3>(radius.val);
	auto gPtcls = generation::generateParticles(fluidVolume.fileName, r, genTechnique::square_grid);
	//{
    ...
    auto vel = fluidVolume.emitter_velocity;
    velocities.push_back(float4_u<SI::velocity>{vel.x, vel.y, vel.z, vel.w});
    fluidVolume.particles_emitted = (int32_t)generated_particles.size();
  }
