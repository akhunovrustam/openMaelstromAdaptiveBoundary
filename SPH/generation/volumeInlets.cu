...
cellFunctionType particleInlet(SPH::streamInlet::Memory arrays, 
	...
	float4_u<SI::m> *positions, float4_u<SI::velocity> velocity, float_u<SI::volume> particleVolume,  float_u<SI::s> delay, float_u<SI::m> distance, float* distanceBuffer, int32_t* type, int32_t* indicator, float4* angularVelocity,
	Ts... tup) {
  checkedThreadIdx(i);
  ...
  if (angularVelocity != nullptr)
    angularVelocity[new_idx] = float4{0.f, 0.f, 0.f, 0.f};
}
...
void SPH::streamInlet::emit(Memory mem) {
  ...
  for (auto &fluidVolume : get<parameters::inlet_volumes::volume>()) {
    //float_u<SI::m> radius = fluidVolume.inlet_radius.value * 1.f / powf(get<parameters::adaptive::resolution>() / 8.f, 1.f / 3.f);
	  float_u<SI::m> radius = fluidVolume.inlet_radius;
      radius = uGet<parameters::particle_settings::radius>();
      if (get<parameters::modules::adaptive>())
          radius = radius / powf(0.25f * get<parameters::adaptive::resolution>(), 1.f / 3.f);
    float_u<SI::volume> particleVolume = PI4O3 * math::power<3>(radius);

    auto num_ptcls = fluidVolume.particles_emitted;
    auto v = fluidVolume.emitter_velocity;
    float4_u<SI::velocity> vel{v.x, v.y, v.z, v.w};

	auto factor = sinf(get<parameters::internal::simulationTime>() * (CUDART_PI_F * 1.f)) * float4_u<SI::velocity>(0.f,5.f,0.f,0.f);
	//vel += factor;

    float_u<SI::s> dur{fluidVolume.duration};
    float_u<SI::s> del{fluidVolume.delay};

    if (dur < get<parameters::internal::simulationTime>() && dur > 0.0_s || del > get<parameters::internal::simulationTime>()) {
      num_ptcl_sum += num_ptcls;
      continue;
    }

    int32_t old_count = get<parameters::internal::num_ptcls>();

    if (old_count + num_ptcls >= get<parameters::simulation_settings::maxNumptcls>())
      continue;
	//std::cout << "-----------------------------------------------------" << std::endl;
	//std::cout << get<parameters::internal::num_ptcls::ptr << std::endl;
	//std::cout << *parameters::internal::num_ptcls::ptr << std::endl;
	//std::cout << old_count << std::endl;
	//std::cout << "######################################################" << std::endl;
    //std::cout << "Particle radius: " << radius.val << std::endl;
    //std::cout << "Particle volume: " << particleVolume.val << std::endl;
	cuda::memcpy(arrays::inletCounter::ptr, &get<parameters::internal::num_ptcls>(), sizeof(int32_t), cudaMemcpyHostToDevice);
    callInlet(sorting_list, mem, num_ptcls, num_ptcl_sum, arrays::inletCounter::ptr, (float4_u<SI::m> *)arrays::inletPositions::ptr, vel,
              particleVolume, get<parameters::adaptive::delay>(), 
        0.f * get<parameters::surfaceDistance::surface_levelLimit>(), arrays::surfaceArrays::distanceBuffer::ptr, 
        arrays::particle_type::ptr, arrays::splitIndicator::ptr, arrays::angularVelocity::ptr );

	//std::cout << *parameters::internal::num_ptcls::ptr << std::endl;
	//std::cout << *parameters::internal::num_ptcls::ptr << std::endl;
	//std::cout << old_count << std::endl;
	cuda::sync("Testing");
	cuda::memcpy(&get<parameters::internal::num_ptcls>(), arrays::inletCounter::ptr, sizeof(int32_t), cudaMemcpyDeviceToHost);
    num_ptcl_sum += num_ptcls;
  }
}
