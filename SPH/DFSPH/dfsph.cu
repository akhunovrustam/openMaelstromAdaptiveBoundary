...
hostDeviceInline auto rhob(const SPH::DFSPH::Memory &arrays, uFloat4<SI::m> position, uFloat<SI::volume> vol,
                           uFloat<SI::density> rho, boundary::kind k = boundary::kind::plane, int32_t idx = -1) {
  return boundary::spline(position, vol, rho, arrays, k, idx);
}
hostDeviceInline auto drhob_dt(const SPH::DFSPH::Memory &arrays, uFloat4<SI::m> position, uFloat4<SI::velocity> v,
                               uFloat<SI::volume> vol, uFloat<SI::density> rho) {
  return (rhob(arrays, position + v * arrays.timestep, vol, rho) - rhob(arrays, position, vol, rho)) / arrays.timestep;
}
hostDeviceInline auto pressureMLS(SPH::DFSPH::Memory &arrays, uFloat4<SI::m> pb, uFloat<SI::volume> vol) {
  ...
  iterateCells(pb, j) {
    ...
    M = M + SVD::Mat3x3{x_j.x * x_j.x, x_j.x * x_j.y, x_j.x * x_j.z, x_j.x * x_j.y, x_j.y * x_j.y,
                        x_j.y * x_j.z, x_j.z * x_j.x, x_j.z * x_j.y, x_j.z * x_j.z} *
                V_j * Wbbf;
...
hostDeviceInline auto gradRho(const SPH::DFSPH::Memory &arrays, uFloat4<SI::m> position, uFloat<SI::volume> vol,
                              uFloat<SI::density> rho, boundary::kind k = boundary::kind::plane, int32_t idx = -1) {
  return boundary::splineGradient(position, vol, rho, arrays, k, idx);
}
hostDeviceInline auto grad2Rho(const SPH::DFSPH::Memory &arrays, uFloat4<SI::m> position, uFloat<SI::volume> vol,
                               uFloat<SI::density> rho, boundary::kind k = boundary::kind::plane, int32_t idx = -1) {
    auto fac = 1.f;// (support_from_volume(1.f) / support_from_volume(vol)).val;
  return boundary::splineGradient2(position, vol, rho, arrays, k, idx) * math::power<5>(fac);
}

#define LOOKUP_DENSITY arrays.density[i]
#define GRADIENT_BOUNDARY                                                                                              \
  boundary::splineGradient(arrays.position[i], arrays.volume[i], LOOKUP_DENSITY, arrays, boundary::kind::plane, -1)
#define GRADIENT2_BOUNDARY                                                                                             \
  boundary::splineGradient2(arrays.position[i], arrays.volume[i], LOOKUP_DENSITY, arrays, boundary::kind::plane, -1)
...
neighFunctionType computeAlphaKernel(SPH::DFSPH::Memory &arrays, bool density, bool boundary) {
  ...
  if (density && boundary)
    kernelSum1 += gradRho(arrays, pos[i], vol[i], arrays.density[i], boundary::kind::plane);
  if (density)
    for (int32_t v = 0; v < arrays.volumeBoundaryCounter; ++v) {
      if (volumeBoundary::distance_fn(arrays.position[i], arrays.volume[i], arrays, v).val.w >= 1e20f)
        continue;
      kernelSum1 += gradRho(arrays, pos[i], vol[i], arrays.density[i], boundary::kind::volume, v);
      if (arrays.volumeBoundaryKind[v] == 0)
        kernelSum2 += grad2Rho(arrays, pos[i], vol[i], arrays.density[i], boundary::kind::volume, v) /
                      arrays.volumeBoundaryDensity[v];
    }

  iterateNeighbors(j) {
    auto kernel = GPW_ij;
    kernelSum1 += arrays.apparentVolume[j] * kernel;
    kernelSum2 += arrays.apparentVolume[j] * arrays.apparentVolume[j] / (arrays.volume[j] * arrays.rest_density) *
                  math::dot3(kernel, kernel);
  }
  uFloat<SI::exp<SI::Pa, -1>> alpha = -arrays.timestep * arrays.timestep * arrays.apparentVolume[i] /
                                          (arrays.volume[i] * arrays.rest_density) *
                                          math::dot3(kernelSum1, kernelSum1) -
                                      arrays.timestep * arrays.timestep * arrays.apparentVolume[i] * kernelSum2;
  arrays.dfsphAlpha[i] = math::min(-1e-20f, alpha);
}
hostDeviceInline auto boundaryPosition(const SPH::DFSPH::Memory &arrays, uFloat4<SI::m> pb, uFloat<SI::volume> v,
                                       boundary::kind k = boundary::kind::plane, int32_t idx = -1) {
...
hostDeviceInline auto boundaryVelocity(const SPH::DFSPH::Memory &arrays, uFloat4<SI::m> pb, uFloat<SI::volume> v,
                                       boundary::kind k = boundary::kind::plane, int32_t idx = -1) {
  if (k == boundary::kind::plane)
    return uFloat4<SI::velocity>{0.f, 0.f, 0.f, 0.f};
  auto rp = boundaryPosition(arrays, pb, v, k, idx);
  return arrays.volumeBoundaryVelocity[idx] +
         math::cross(arrays.volumeBoundaryAngularVelocity[idx].val, rp.first.val - rp.second.val);
}
hostDeviceInline auto boundaryAcceleration(const SPH::DFSPH::Memory &arrays, uFloat4<SI::m> pb, uFloat<SI::volume> v,
                                           boundary::kind k = boundary::kind::plane, int32_t idx = -1) {
  if (k == boundary::kind::plane)
    return uFloat4<SI::acceleration>{0.f, 0.f, 0.f, 0.f};
  auto rp = boundaryPosition(arrays, pb, v, k, idx);
  return (arrays.volumeBoundaryKind[idx] != 0 ? 0.f : 1.f) *
         (arrays.volumeBoundaryAcceleration[idx] +
...
neighFunctionType computeSourceTermKernel(const SPH::DFSPH::Memory &arrays, bool density, bool boundary) {
  ...
  if (density && boundary)
    sourceTerm =
        sourceTerm - arrays.timestep * math::dot3(arrays.velocity.first[i], GRADIENT_BOUNDARY) / (BOUNDARY_DENSITY);
  ...
  if (density)
    for (int32_t v = 0; v < arrays.volumeBoundaryCounter; ++v) {
      ...
      sourceTerm =
          sourceTerm - arrays.timestep * math::dot3(arrays.velocity.first[i] - vel,
                                                    gradRho(arrays, pos[i], arrays.volume[i], arrays.density[i],
                                                            boundary::kind::volume, v));
    }
  ...
  iterateNeighbors(j) sourceTerm =
      sourceTerm - arrays.timestep * arrays.apparentVolume[j] *
                       math::dot3(arrays.velocity.first[i] - arrays.velocity.first[j], GPW_ij);
...
neighFunctionType computeAccelerationKernel(const SPH::DFSPH::Memory arrays, bool density, bool boundary) {
  ...
  iterateNeighbors(j) kernelSum += -arrays.volume[j] * arrays.rest_density *
                                   (pressure[i] / math::square(arrays.density[i] * arrays.rest_density) +
                                    pressure[j] / math::square(arrays.density[j] * arrays.rest_density)) *
                                   GPW_ij;
...
neighFunctionType updatePressureKernel(const SPH::DFSPH::Memory arrays, bool density, bool boundary) {
  ...
  alias_arrays((i_vol, apparentVolume));
  ...
  if (density)
    for (int32_t v = 0; v < arrays.volumeBoundaryCounter; ++v) {
      ...
      kernelSum += arrays.timestep * arrays.timestep *
                   math::dot3(acc[i] - bacc, gradRho(arrays, pos[i], arrays.volume[i], arrays.density[i],
                                                     boundary::kind::volume, v));
    }
  ...
  // omega = 0.5f;
  uFloat<SI::Pa> pressure =
      arrays.pressure.first[i] + omega / arrays.dfsphAlpha[i] * (arrays.dfsphSource[i] - kernelSum);
  if (density)
    pressure = math::max(uFloat<SI::Pa>{0.f}, pressure);

  uFloat<> residual = kernelSum - arrays.dfsphSource[i];

  if (math::abs(arrays.dfsphAlpha[i]) < 1e-10f || pressure != pressure || pressure > 1e15f) {
    pressure = 0._Pa;
    residual = decltype(residual)(0.f);
  }
   
  arrays.pressure.second[i] = pressure;
  //arrays.dfsphDpDt[i] = math::max(residual.val, -0.005f) * arrays.volume[i].val;
  arrays.dfsphDpDt[i] =residual.val * arrays.volume[i].val;
   //arrays.dfsphRhoStar[i] = math::max(residual, 0.f) * arrays.volume[i].val;
  // arrays.dfsphRhoStar[i] = residual * arrays.volume[i].val ; 
  arrays.dfsphRhoStar[i] = math::max(residual, -0.00f) * arrays.volume[i].val; /// arrays.rest_density.val;
}

template <neighbor_list neighborhood>
hostDeviceInline auto calculateBoundaryPressure(int32_t i, float4_u<SI::m> pb, SPH::DFSPH::Memory &arrays,
                                                bool density, int32_t b, boundary::kind k) {
  float4 vecSum{0.f, 0.f, 0.f, 0.f};
  SVD::Mat3x3 M{0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f};
  float sumA = 0.f;
  float sumB = 0.f;
  ...
  iterateCells(pb, j) {
    auto pj = arrays.position[j];
    auto fac = arrays.volume[j].val * kernel(pj, pb);
    d_bar += pj.val * fac;
    d_sum += fac;
  }
  d_bar /= d_sum;
  ...
  auto det = M.det();
  if (det != det
      || math::abs(det) < 1e-4f || fabsf(svd.S.m_00) < 1e-8f || fabsf(svd.S.m_11) < 1e-8f || fabsf(svd.S.m_22) < 1e-8f
  ) {
    beta = gamma = delta = 0.f;
  }
  auto pressure = alpha * 1.f + beta * x_b.x + gamma * x_b.y + delta * x_b.z;
...
neighFunctionType boundaryMLSPressureKernel(SPH::DFSPH::Memory arrays, bool density, bool boundary,
                                            int32_t iteration) {
  // if (!density) return;
  checkedParticleIdx(i);
  ...
  if (density && boundary)
    if (boundaryDistance.w < 1e20f) {
      ...
      float_u<SI::Pa> boundaryPressure{
          calculateBoundaryPressure<neighborhood>(i, pb, arrays, density, -1, boundary::kind::plane)};
      ...
      auto boundaryKernel = gradRho(arrays, x_i, V_i, arrays.density[i]);
      // boundaryKernel = boundary::splineGradient(arrays.position[i] + arrays.velocity.first[i] * arrays.timestep,
      // arrays.volume[i], LOOKUP_DENSITY, arrays, boundary::kind::plane, -1); if (iteration > 0)
      accel += -1.f * arrays.rest_density *
               (fluid / math::square(arrays.density[i] * arrays.rest_density) +
                boundary / math::square(arrays.density[i] * arrays.rest_density)) *
  ...
  if (density)
      for (int32_t v = 0; v < arrays.volumeBoundaryCounter; ++v) {
          ...
          if (d < 1e20f) {
              float_u<SI::Pa> boundaryPressure{
                  calculateBoundaryPressure<neighborhood>(i, pb, arrays, density, v, boundary::kind::volume) };
              auto fluid = math::max(0.0_Pa, arrays.pressure.second[i]);
              auto boundary = math::max(0.0_Pa, boundaryPressure);
              auto boundaryKernel = gradRho(arrays, x_i, V_i, arrays.density[i], boundary::kind::volume, v);
             // if (math::dot3(boundaryKernel, accel) < 0.f)
              //    boundaryKernel = math::length3(boundaryKernel) * math::normalize(math::normalize(accel).val + math::normalize(boundaryKernel).val);
              ...
              torque.w = 0.f;
              auto angularAccel =
                  arrays.volumeBoundaryTransformMatrix[v] *
...
neighFunction(computeBoundaryPressure, boundaryMLSPressureKernel, "DFSPH: MLS", caches<float4, float>{});
...
neighFunctionType boundaryFrictionKernel(SPH::DFSPH::Memory &arrays, bool density, int32_t iteration) {
  checkedParticleIdx(i);
  ...
  if (boundaryDistance.w < 1e20f) {
    ...
    float_u<SI::Pa> boundaryPressure{
        calculateBoundaryPressure<neighborhood>(i, pb, arrays, density, -1, boundary::kind::plane)};
    auto boundaryKernel = boundary::splineGradient(arrays.position[i], arrays.volume[i], LOOKUP_DENSITY, arrays,
                                                   boundary::kind::plane, -1);
    float4_u<SI::N> F_p = -arrays.volume[i] * arrays.rest_density * arrays.rest_density *
                          (arrays.pressure.second[i] / math::square(arrays.density[i] * arrays.rest_density) +
                           boundaryPressure / math::square(arrays.rest_density)) *
                          (boundaryKernel);
...
neighFunctionType volumeFrictionKernel(SPH::DFSPH::Memory &arrays, bool density, int32_t iteration, int32_t b) {
  checkedParticleIdx(i);
  alias_arrays((pos, position));
  ...
  if (d < 1e20f) {
    float_u<SI::Pa> boundaryPressure{
        calculateBoundaryPressure<neighborhood>(i, pb, arrays, density, b, boundary::kind::volume)};
    auto fluid = math::max(0.0_Pa, arrays.pressure.second[i]);
    auto boundary = math::max(0.0_Pa, boundaryPressure);
    auto boundaryKernel = gradRho(arrays, x_i, arrays.volume[i], arrays.density[i], boundary::kind::volume, b);

    auto F_p =
        -arrays.volume[i] * arrays.rest_density * arrays.rest_density *
        (fluid / math::square(arrays.density[i] * arrays.rest_density) + boundary / math::square(arrays.rest_density)) *
        boundaryKernel; // *(density ? 1.f : 0.f);

    auto v_b =
        arrays.volumeBoundaryVelocity[b].val - math::cross(pb - com, arrays.volumeBoundaryAngularVelocity[b].val);
    // v_b = -v_b;
    uFloat4<> n_i = math::normalize3(F_p);
    uFloat4<SI::velocity> v_ib = v_i - v_b;
    ...
    if (a_i.val.x == a_i.val.x && a_i.val.y == a_i.val.y && a_i.val.z == a_i.val.z) {
      ...
      torque.w = 0.f;
      auto angularAccel =
          arrays.volumeBoundaryTransformMatrix[b] *
...
neighFunction(boundaryFriction, boundaryFrictionKernel, "DFSPH: compute alpha");
neighFunction(volumeFriction, volumeFrictionKernel, "DFSPH: compute alpha");
...
void SPH::DFSPH::density_solve(Memory arrays) {
  if (arrays.num_ptcls == 0)
    return;
  static auto prepareTimer = TimerManager::createGPUTimer("Incompressible: prepare", Color::anjou_pear, __FILE__, __LINE__);
  static auto predictTimer = TimerManager::createGPUTimer("Incompressible: predict Velocity", Color::anjou_pear, __FILE__, __LINE__);
  static auto alphaTimer = TimerManager::createGPUTimer("Incompressible: compute alpha", Color::anjou_pear, __FILE__, __LINE__);
  static auto sourceTimer = TimerManager::createGPUTimer("Incompressible: compute source", Color::anjou_pear, __FILE__, __LINE__);
  static auto solveTimer = TimerManager::createGPUTimer("Incompressible: solve", Color::anjou_pear, __FILE__, __LINE__);
  static auto boundaryTimer =
      TimerManager::createGPUTimer("Incompressible: boundary pressure", Color::anjou_pear, __FILE__, __LINE__);
  static auto accelTimer = TimerManager::createGPUTimer("Incompressible: compute accel", Color::anjou_pear, __FILE__, __LINE__);
  static auto velocityTimer = TimerManager::createGPUTimer("Incompressible: update velocity", Color::anjou_pear, __FILE__, __LINE__);
  static auto frictionTimer = TimerManager::createGPUTimer("Incompressible: friction", Color::anjou_pear, __FILE__, __LINE__);
  static auto totalTimer = TimerManager::createGPUTimer("Incompressible: total", Color::anjou_pear, __FILE__, __LINE__);
  totalTimer->start();
  prepareTimer->start();
  static bool once = true;
  static float4 *torqueTemp = nullptr;
  static float4 *forceTemp = nullptr;
  auto nb = get<parameters::boundary_volumes::volumeBoundaryCounter>();
  if (nb > 0 && once) {
    once = false;
  ...
  if (nb > 0) {
    ...
    cudaMemcpy(forceTemp, arrays::volumeBoundaryAngularAcceleration::ptr, sizeof(float4) * nb,
  ...
  sourceTimer->stop();
  solveTimer->start();
  auto &error = get<parameters::dfsph_settings::densityError>();
  auto &itCounter = get<parameters::dfsph_settings::densitySolverIterations>();
  itCounter = 0;
  ...
  do {
    if (nb > 0) {
      cudaMemcpy(arrays::volumeBoundaryAngularAcceleration::ptr, torqueTemp, sizeof(float4) * nb,
                 cudaMemcpyDeviceToDevice);
  ...
  } while (itCounter++ < 1 || (error > get<parameters::dfsph_settings::densityEta>() && itCounter < 256));
  if (nb > 0) {
    cudaMemcpy(arrays::volumeBoundaryAngularAcceleration::ptr, torqueTemp, sizeof(float4) * nb,
               cudaMemcpyDeviceToDevice);
    cudaMemcpy(arrays::volumeBoundaryAcceleration::ptr, forceTemp, sizeof(float4) * nb, cudaMemcpyDeviceToDevice);
...
void SPH::DFSPH::divergence_solve(Memory arrays) {
  if (arrays.num_ptcls == 0)
    return;
  bool planeDivergence = get<parameters::moving_plane::plane>().size() == 0;
  ...
  if (nb > 0) {
    cudaMemcpy(torqueTemp, arrays::volumeBoundaryAngularAcceleration::ptr, sizeof(float4) * nb,
               cudaMemcpyDeviceToDevice);
  ...
  cuda::sync();
  auto &error = get<parameters::dfsph_settings::divergenceError>();
  auto &itCounter = get<parameters::dfsph_settings::divergenceSolverIterations>();
  itCounter = 0;
  float totalVolume = math::getValue(algorithm::reduce_sum(arrays.volume, arrays.num_ptcls));
  cuda::sync();
  // std::cout << "Total volume in frame " << get<parameters::internal::frame>() << " = " << totalVolume << std::endl;
  do {
    if (nb > 0) {
      cudaMemcpy(arrays::volumeBoundaryAngularAcceleration::ptr, torqueTemp, sizeof(float4) * nb,
                 cudaMemcpyDeviceToDevice);
    ...
    itCounter++;
  } while (itCounter < 2 || error > get<parameters::dfsph_settings::divergenceEta>() && itCounter < 256);
  if (nb > 0) {
    cudaMemcpy(arrays::volumeBoundaryAngularAcceleration::ptr, torqueTemp, sizeof(float4) * nb,
               cudaMemcpyDeviceToDevice);
    cudaMemcpy(arrays::volumeBoundaryAcceleration::ptr, forceTemp, sizeof(float4) * nb, cudaMemcpyDeviceToDevice);
  ...
  launch<computeAcceleration>(arrays.num_ptcls, arrays, false, planeDivergence);
  cuda::sync();
  launch<updateVelocity>(arrays.num_ptcls, arrays, arrays::lifetime::ptr,
                         FLT_MAX); // get<parameters::adaptive::delay>());
  cuda::sync();
...
void SPH::DFSPH::print_velocity(Memory arrays) {
   /* for (int i = 0; i < arrays.num_ptcls; i++)
    {
        if (arrays.particle_type[i] == 0)
        {
            auto vel = arrays.velocity.first[i].val;
            std::cout << "vel " << vel.x << " " << vel.y << " " << vel.z << std::endl;
        }
    }*/
            std::cout << "vel -------------------" << std::endl;
    auto mind = get<parameters::internal::min_domain>();
    auto maxd = get<parameters::internal::max_domain>();
    std::cout << " mind " << mind.x << " " << mind.y << " " << mind.z << " " << " maxd " << maxd.x << " " << maxd.y << " " << maxd.z << std::endl;
}