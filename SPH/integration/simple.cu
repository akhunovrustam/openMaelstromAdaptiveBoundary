...
void updateRigids(SPH::Integration::Memory mem) {
  ...
  // static int32_t* particle_type = (int32_t*) malloc(sizeof(int32_t) * get<parameters::simulation_settings::maxNumptcls>());
  // cudaMemcpy(particle_type, mem.particle_type, sizeof(int32_t) * get<parameters::simulation_settings::maxNumptcls>(), cudaMemcpyDeviceToHost);

  cudaDeviceSynchronize();
  static auto pos = std::make_unique<float4_u<SI::m>[]>(mem.maxNumptcls);
  static auto accels = std::make_unique<float4_u<SI::acceleration>[]>(mem.maxNumptcls);
  static auto volumes = std::make_unique<float_u<SI::volume>[]>(mem.maxNumptcls);
  static auto types = std::make_unique<int32_t[]>(mem.maxNumptcls);
  ...
  DynamicsWorld::getInstance()->stepSimulation(get<parameters::internal::timestep>());

  // auto cntr = body->getCenterOfMassPosition();
  // std::cout << "center mass: " << cntr.getX() << " " << cntr.getY() << " " << cntr.getZ() << std::endl;
  // std::cout << "timestep: " << get<parameters::internal::timestep>() << std::endl;
  // auto body
...
basicFunctionType velocity_update(SPH::Integration::Memory arrays) {
  ...
  if (arrays.particle_type[i] != 0) {
	  arrays.velocity.second[i] = { 0, 0, 0, 0 };
	  arrays.velocity.first[i] = { 0, 0, 0, 0 };
	  return;
  }
  ...
  /*printf("final final acc %f %f %f\n", a.val.x, a.val.y, a.val.z);
  printf("final velocity acc %f %f %f\n", v.val.x, v.val.y, v.val.z);
  */v += dt * a;
  v.val.w = 0.f;
...
basicFunctionType update_energy(SPH::Integration::Memory arrays, float* potential, float* kinetic) {
    checkedParticleIdx(i);

    potential[i] = arrays.position.first[i].val.z * arrays.volume[i].val * 9.98f * arrays.rest_density.val;

    auto vel = math::length3(arrays.velocity.first[i]).val;
    kinetic[i] = 0.5f * arrays.volume[i].val * arrays.rest_density.val * vel * vel;

}
basicFunction(updateEnergy, update_energy, "Integrate: Energy");
...
void updateVolumeBoundary(float dt) {
	if (get<parameters::boundary_volumes::volumeBoundaryCounter>() == 0) {
		return;
	}
	int32_t n = get<parameters::boundary_volumes::volumeBoundaryCounter>();
	...
	for (int32_t b = 0; b < get<parameters::boundary_volumes::volumeBoundaryCounter>(); ++b) {
		...
		//totalR = totalR / 2.f;
		float3 totalR = math::castTo<float3>(vols[b].angularVelocity);
		float3 totalV = vols[b].velocity * dt;
		...
		if (get<parameters::boundary_volumes::volume>()[b].kind == 1) {
...
void SPH::Integration::update_positions(Memory mem) {
  ...
  //if (get<parameters::internal::simulationTime>() > 1.f)
	 // for (int i = 0; i < mem.num_ptcls; i++)
	 // {
		//  if (mem.particle_type[i] == 0) mem.position.first[i].val.z = 100.f;
	 // }
  //static float* potentialEnergyArr = nullptr;
  //static float* kineticEnergyArr = nullptr;
  //if (potentialEnergyArr == nullptr) {
  //    cudaMalloc(&potentialEnergyArr, sizeof(float) * mem.maxNumptcls);
  //    cudaMalloc(&kineticEnergyArr, sizeof(float) * mem.maxNumptcls);
  //}
  //static float oldTime = -1.f;
  //if (get<parameters::internal::simulationTime>() > oldTime + 1.f / 60.f) {
  //    oldTime = get<parameters::internal::simulationTime>();
  //    launch<updateEnergy>(mem.num_ptcls, mem, potentialEnergyArr, kineticEnergyArr);
  //    float potentialEnergy = algorithm::reduce_sum(potentialEnergyArr, mem.num_ptcls);
  //    float kineticEnergy = algorithm::reduce_sum(kineticEnergyArr, mem.num_ptcls);
  //    std::cout << get<parameters::internal::frame>() << " " << get<parameters::internal::simulationTime>() << " "
  //        << get<parameters::internal::num_ptcls>() << " " << potentialEnergy << " " << kineticEnergy << std::endl;
  //}
  ...
  // dt = math::min(dt, get<parameters::simulation_settings::timestep_max>());
  int32_t n = get<parameters::boundary_volumes::volumeBoundaryCounter>();
  if (n > 0) {
	  std::vector<float4> velocities(n), angularVelocities(n);
  ...
  dt = math::clamp(dt, get<parameters::simulation_settings::timestep_min>(), get<parameters::simulation_settings::timestep_max>());
  // dt = math::clamp(dt, get<parameters::internal::timestep>() * 0.9f, get<parameters::internal::timestep>() * 1.1f);

  uGet<parameters::internal::timestep>() = dt;
}
...
void SPH::Integration::update_velocities(Memory mem) {
  ...
  uGet<parameters::internal::max_velocity>() = algorithm::reduce_max(mem.maxVelocity, mem.num_ptcls);
}
