...
public:
	...
	template<typename T>
	std::enable_if_t<detail::is_specialization<T, std::vector>::value, std::vector<typename T::value_type>&> uGet(std::string identifier) {
		return boost::any_cast<std::vector<typename T::value_type>&>(getParameter(identifier).param.valVec.value());
	}
	template<typename T>
	std::enable_if_t<!detail::is_specialization<T, std::vector>::value, T&> uGet(std::string identifier) {
		auto& param = getParameter(identifier);
		boost::any& any = param.param.val.value();
		if constexpr (detail::is_specialization<T, value_unit>::value) {
			typename T::type& intermediate = boost::any_cast<typename T::type&>(any);
			if (std::count(param.properties.alternativeTypes.begin(), param.properties.alternativeTypes.end(), typeid(T)) == 0)
				throw std::invalid_argument(std::string("Invalid cast to unit type ") + typeid(T).name() + " for " + identifier);
...
#include <math/math.h>
struct gvdbHelper {
	static nvdb::VolumeGVDB& getNewInstance();
	static void initialize();
	static std::tuple< VDBInfo*, float4, float4> loadIntoGVDB(std::string fileName);
private:
	static std::vector<nvdb::VolumeGVDB> gvdbInstances;
};