...
struct Box {
  ...
  __device__ float intersect(const worldRay &worldRay) const {
    float tmin = ((worldRay.dir.x < 0.f ? max.x : min.x) - worldRay.orig.x) / worldRay.dir.x;
    float tmax = ((worldRay.dir.x < 0.f ? min.x : max.x) - worldRay.orig.x) / worldRay.dir.x;
    float tymin = ((worldRay.dir.y < 0.f ? max.y : min.y) - worldRay.orig.y) / worldRay.dir.y;
    float tymax = ((worldRay.dir.y < 0.f ? min.y : max.y) - worldRay.orig.y) / worldRay.dir.y;
...
struct Sphere {
  ...
  __device__ float intersect(const worldRay &r) const {
    float3 op = pos - r.orig; //
    float t, epsilon = 0.01f;
    float b = math::dot(op, r.dir);
    float disc = b * b - math::dot(op, op) + rad * rad; // discriminant
...
public:
  vRTXrender();
  virtual void uiFunction() override;
  virtual std::map<std::string, std::variant<int32_t, float, std::vector<int32_t>, std::vector<float>>> getData() override;
  ...
  vrtx::Refl_t fluidMaterial = vrtx::Refl_t::Nayar;
...
cuVRTXRender(SceneInformation scene, cudaGraphicsResource_t resource, vrtx::objectLoader &sceneMeshes,
             vrtxFluidMemory fmem, vrtxFluidArrays farrays, float3 *accumulateBuffer, unsigned framenumber,
             unsigned hashedframes, int32_t renderMode, int32_t bounces, bool fluidRender, int32_t renderGrid,
             int32_t surfacingTechnique, vrtx::Refl_t fluidMaterial, bool dirty, std::vector<vrtx::Sphere> spheres,
             std::vector<vrtx::Box> boxes);