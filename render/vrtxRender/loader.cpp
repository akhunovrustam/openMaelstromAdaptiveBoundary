...
void vrtx::objectLoader::appendObject(std::string fileName) {
  ...
  auto file = resolveFile(fileName, { get<parameters::internal::config_folder>() });
  auto f_obj = file;
  f_obj.replace_extension(".obj");

  auto [objVertices, objTriangles, edges, minp, maxp] = generation::fileToObj(std::filesystem::exists(f_obj) ? f_obj : file);
...
void vrtx::objectLoader::appendObject(int32_t idx) {
	auto bobj = get<parameters::boundary_volumes::volume>()[idx];
	LOG_INFO << "Started loading object " << bobj.fileName << std::endl;
	auto file = resolveFile(bobj.fileName, { get<parameters::internal::config_folder>() });
	auto f_obj = file;
	f_obj.replace_extension(".obj");
	mesh m;
