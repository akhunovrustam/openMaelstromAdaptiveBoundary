...
template <typename T> hostDeviceInline auto cbrt(T &&lhs) {
  // rhs and U are required for the fn_wrapper_2_U macro to work.
  //auto rhs = ((float)R::num) / ((float)R::den);
  //using U = decltype(rhs);
 // auto rhs = ((float)1) / ((float)3);
 // using U = decltype(rhs);
  return fn_wrapper_1_U({ return ::cbrtf(a); }, (SI::multiply_ratios_ty<T, SI::ratio<1, 3>>));
}
...
template <typename T> hostDeviceInline auto length(T &&value) { return sqrt(math::dot(value, value)); }
/* Square Length of a vector as the dot product with itself */
template <typename T> hostDeviceInline auto sqlength(T &&value) { return (math::dot(value, value)); }
/* Length of a vector as the sqrt of the dot3 product with itself */
template <typename T> hostDeviceInline auto length3(T &&value) { return sqrt(math::dot3(value, value)); }
/* Square Length of a vector as the dot3 product with itself */
template <typename T> hostDeviceInline auto sqlength3(T &&value) { return (math::dot3(value, value)); }
/* Normalizes a vector by dividing it with the length of itself.
 * DOES NOT CHECK FOR ZERO LENGTH!*/
template <typename T> hostDeviceInline auto normalize(T &&value) { return value / math::length(value); }
/* Normalizes a vector by dividing it with the length3 of itself.
 * DOES NOT CHECK FOR ZERO LENGTH! Applies to all dimensions. */
template <typename T> hostDeviceInline auto normalize3(T &&value) { return value / math::length3(value); }
...
template <typename T, typename U> hostDeviceInline auto reflect(T &&lhs, U &&rhs) {
  return lhs - 2.f * rhs * math::dot(rhs, lhs);
}
