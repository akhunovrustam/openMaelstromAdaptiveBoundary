...
#include <math/template/for_struct.h>
...
template<typename Ty> struct moduleCall : public simulationStep{
	...
	virtual void call() {
		...
		if (timer == nullptr && has_timer)
			timer = TimerManager::createGPUTimer(description, color, __FILE__, __LINE__);
		...
		/*try {
			...
			for (int i = 0; i < get<parameters::internal::num_ptcls>(); i++)
				if (arrays::particle_type::ptr[i] == 0) cnt++;
			std::cout << description << "\n";
			std::cout << "---FLUIDS444: " << cnt << "\n";
		} catch(...){
		...
		if (this->timer)
		this->timer->stop();
		if (get<parameters::modules::error_checking>()) {
...
void cuda_particleSystem::setup_simulation() {
	functions.push_back(new moduleCall( &SPH::compactMLM::markBegin, "compactMLM sort: markBegin", Color::aquarium));
	functions.push_back(new moduleCall(&SPH::detection::showBuffer, "Surface detection", Color::presidential_blue));
	functions.push_back(new moduleCall( &SPH::IISPH17_PRESSURE_MIRRORING_D::print_begin, "IISPH17 Stefan Band rigid bodies: incompressibility", Color::greencopper));
	//functions.push_back(new moduleCall( &SPH::IISPH17_PRESSURE_MIRRORING_D::print_velocity, "IISPH17 Stefan Band rigid bodies: incompressibility", Color::greencopper));
	//functions.push_back(new moduleCall( &SPH::DFSPH::print_velocity, "IISPH17 Stefan Band rigid bodies: incompressibility", Color::greencopper));
	functions.push_back(new moduleCall( &SPH::compactMLM::calculateFluid, "compactMLM sort: calculate fluid", Color::aquarium));
	...
	//functions.push_back(new moduleCall( &SPH::IISPH17_PRESSURE_MIRRORING_D::print_velocity, "IISPH17 Stefan Band rigid bodies: incompressibility", Color::greencopper));
	functions.push_back(new moduleCall( &SPH::IISPH17_PRESSURE_MIRRORING_D::pressure_solve, "IISPH17 Stefan Band rigid bodies: incompressibility", Color::greencopper));
	functions.push_back(new moduleCall( &SPH::IISPH17_PRESSURE_BOUNDARIES_D::pressure_solve, "IISPH17 Stefan Band rigid bodies: incompressibility", Color::greencopper));
	//functions.push_back(new moduleCall( &SPH::IISPH17_PRESSURE_MIRRORING_D::print_velocity, "IISPH17 Stefan Band rigid bodies: incompressibility", Color::greencopper));
 ...
 //   functions.push_back(new moduleCall(&SPH::LiuVorticity::vorticity, "Vorticity: Liu", Color::amethyst));
	...
	if (get<parameters::modules::adaptive>() == true || get<parameters::inlet_volumes::volume>().size() > 0) {
		//// calculate density
		functions.push_back(new moduleCall(&SPH::Resort::resortParticles, "Linear Sort: resort", Color::aquaman));
		functions.push_back(new moduleCall(&SPH::resort_mlm::resortParticles, "MLM sort: resort", Color::aquarium));
		functions.push_back(new moduleCall(&SPH::compactMLM::resortParticles, "compactMLM sort: resort", Color::aquarium));
	...
	functions.push_back(new moduleCall(&SPH::detection::fluidneighbor, "Surface detection", Color::presidential_blue));
	functions.push_back(new moduleCall( &SPH::compactMLM::calculateFluid, "compactMLM sort: calculate fluid", Color::aquarium));
	
	functions.push_back(new moduleCall( &SPH::adaptive::calculateVol, "calculate total volume of particles", Color::aquarium));
	
	//functions.push_back(new moduleCall(&SPH::adaptive::copydensity, "Adaptivity: copy density", Color::braeburn_apple));
	functions.push_back(new moduleCall( &SPH::compactMLM::calculateFluid, "compactMLM sort: calculate fluid", Color::aquarium));
	functions.push_back(new moduleCall(&SPH::adaptive::adapt, "Adaptivity: adapt", Color::braeburn_apple));
	functions.push_back(new moduleCall( &SPH::adaptive::calculateVol, "calculate total volume of particles", Color::aquarium));
	functions.push_back(new moduleCall( &SPH::compactMLM::calculateFluid, "compactMLM sort: calculate fluid", Color::aquarium));

	if (get<parameters::modules::rayTracing>() == true || get<parameters::modules::adaptive>() == true) {
		functions.push_back(new moduleCall(&SPH::Resort::resortParticles, "Linear Sort: resort", Color::aquaman));
		functions.push_back(new moduleCall(&SPH::resort_mlm::resortParticles, "MLM sort: resort", Color::aquarium));
		functions.push_back(new moduleCall(&SPH::compactMLM::resortParticles, "compactMLM sort: resort", Color::aquarium));
		functions.push_back(new moduleCall(&SPH::enforceSymmetry::constrain_support, "Constraining Support", Color::braeburn_apple));
	...
	//functions.push_back(new moduleCall(&SPH::adaptive::removeexcess, "Adaptivity: adapt", Color::braeburn_apple));
	functions.push_back(new moduleCall( &SPH::compactMLM::calculateFluid, "compactMLM sort: calculate fluid", Color::aquarium));
	...
	{
	functions.push_back(new moduleCall( &SPH::compactMLM::calculateFluid, "compactMLM sort: calculate fluid", Color::aquarium));
		functions.push_back(new moduleCall(&SPH::adaptive::postcontoptimizeCount, "Adaptivity: optimize boundary particles after splitting/mergin", Color::braeburn_apple));
		//functions.push_back(new moduleCall(&SPH::adaptive::postcontoptimize, "Adaptivity: optimize boundary particles after splitting/mergin", Color::braeburn_apple));
	//functions.push_back(new moduleCall( &SPH::compactMLM::calculateFluid, "compactMLM sort: calculate fluid", Color::aquarium));
		//functions.push_back(new moduleCall(&SPH::adaptive::postoptimize, "Adaptivity: optimize boundary particles after splitting/mergin", Color::braeburn_apple));
	functions.push_back(new moduleCall( &SPH::compactMLM::calculateFluid, "compactMLM sort: calculate fluid", Color::aquarium));
		functions.push_back(new moduleCall(&SPH::adaptive::postoptimizemerge, "Adaptivity: optimize boundary particles after splitting/mergin", Color::braeburn_apple));
	}
	//functions.push_back(new moduleCall( &SPH::IISPH17_PRESSURE_MIRRORING_D::print_velocity, "IISPH17 Stefan Band rigid bodies: incompressibility", Color::greencopper));
        functions.push_back(new moduleCall(&SPH::Visualizer::visualizeParticles, "Visualize", Color::neonpink));
#ifdef ANISOTROPIC_SURFACE
        functions.push_back(
            new moduleCall(&SPH::anisotropy::generateAnisotropicMatrices, "Anisotropy", Color::aquarium));
#endif
        functions.push_back(new moduleCall(&SPH::renderMLM::generateAuxilliaryGrid, "renderMLM", Color::jolly_green));
	functions.push_back(new moduleCall( &SPH::compactMLM::calculateFluid, "compactMLM sort: calculate fluid", Color::aquarium));
	
	//functions.push_back(new moduleCall( &SPH::IISPH17_PRESSURE_MIRRORING_D::print_velocity, "IISPH17 Stefan Band rigid bodies: incompressibility", Color::greencopper));
	functions.push_back(new moduleCall(&SPH::logDump::log_dump, "Log dump", Color::braeburn_apple));
...
void cuda_particleSystem::storeDebugArrays() {
	if (get<parameters::modules::debug>() == false) return;
...
void cuda_particleSystem::init_simulation() {
  IO::config::load_config(get<parameters::internal::config_file>());
  IO::config::show_config();
  if (arguments::cmd::instance().snapped)
	  get<parameters::modules::gl_record>() = false;
  SPH::volume::init_volumes();
  if(!arguments::cmd::instance().snapped)
  IO::vdb::emitParticleVolumes();
  else if(get<parameters::rigid_volumes::volume>().size() > 0) IO::vdb::recreateRigids();
  ...
  if (get<parameters::modules::debug>() == true) {
	  for_struct(m_debugArrays, [&](auto& arr) {
		  using Ta = std::decay_t<decltype(arr)>;
		  using Ty = typename Ta::value_type;
		  arr.valid = Ty::valid();
	...
	if (once) {
		...
		if (renderFlag && (running || single)) {
		{
			...
			TIME_CODE(Frame, Color::neonpink,
				get<parameters::internal::simulationTime>() += get<parameters::internal::timestep>();
			static bool oncee = true;
			if (oncee) {
				//arguments::cmd::instance().logParameters();
				oncee = false;
			}
			for (auto& fun : functions)
			{
				...
				/*try {
					...
					for (int i = 0; i < get<parameters::internal::num_ptcls>(); i++)
						if (arrays::particle_type::ptr[i] == 0) cnt++;
					std::cout << fun->description << "\n";
					std::cout << "---FLUIDS444: " << cnt << "\n";
				} catch(...){
					
				}*/
			}
			if (get<parameters::modules::alembic>()) {
			...
			get<parameters::internal::frame>() += 1;
			);
			static Timer* simTimer = nullptr;
			if (simTimer == nullptr) {
				...
				ParameterManager::instance().get<float>("simTime") = *simTimer->getSamples().rbegin();
			}
		...
		static float last_write = get<parameters::internal::simulationTime>() - 1.f / get<parameters::render_settings::camera_fps>();
		if (arguments::cmd::instance().outputSimulation &&
			(get<parameters::internal::simulationTime>() >=
				last_write + 1.f / get<parameters::render_settings::camera_fps>())
			) {
			// std::cout << "Writing data at " << get<parameters::internal::simulationTime>() << ", prior frame: " << last_write << " interval " << 1.f / get<parameters::render_settings::camera_fps>() << std::endl;
			IO::renderData::saveParticles();
			last_write += 1.f / get<parameters::render_settings::camera_fps>();
		}
		//std::this_thread::sleep_for(std::chrono::milliseconds(100));
  }
  if(get<parameters::internal::dumpNextframe>()){
	  get<parameters::internal::dumpNextframe>() = 0;
	  IO::dumpAll(get<parameters::simulation_settings::dumpFile>());
  }
}
