	...
	static void leanAllocate();)";
		...
		text_fn.push_back([&](auto text) {
			...
			if (kind != "customData") {
				...
				if (kind == "particleData") size_str = "(get<parameters::simulation_settings::maxNumptcls>() + 1)";
				if (kind == "diffuseData") size_str = "get<parameters::max_diffuse_numptcls>()";
				if (kind == "cellData") size_str = "get<parameters::internal::gridSize>().x * get<parameters::internal::gridSize>().y * get<parameters::internal::gridSize>().z";
				swap_str = R"(
